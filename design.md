...
| 文件名 | 类型 | 职能描述 |
| :--- | :--- | :--- |
| `tcl_core.c` | C 源码 | **解释器核心**：包含类型系统、FSM 状态机、内存 Arena 管理及 18 个原初原子指令。严禁依赖任何标准 C 库。 |
| `demo.c` | C 源码 | **测试外壳**：通过 `#include "tcl_core.c"` 直接集成内核，提供 Linux 环境下的交互式 REPL 和文件执行接口。 |
...
### 10.2 审查与反馈循环 (Review & Feedback Loop)
1.  **任务下发**：主 Agent 将任务分解，并向子 Agent 提供上下文。
2.  **阶段性提交与报告 (Staged Commits & Reporting)**：
    *   子 Agent 必须在完成逻辑阶段后进行**本地 Git 提交**，确保版本历史清晰。
    *   子 Agent 在提交审查时，必须附带一份**实现报告**，包含：
        *   **已解决的问题**：具体描述完成了哪些功能或修复了哪些 Bug。
        *   **设计思路**：阐述为何采用该方案，如何避开了“物理红线”和“栈溢出陷阱”。
        *   **后续待办**：列出尚未完成或需要进一步完善的模块。
        *   **风险预警**：指出代码中潜在的薄弱环节、需要注意的边界情况，以便主 Agent 识别“幻觉”或设计偏离。
3.  **对齐校验与执行安全**：
    *   主 Agent 依据报告和代码，对照 `design.md` 进行逐行核对。
    *   **执行安全 (Execution Safety)**：任何阶段的自动化测试运行（特别是 `./tclsh`）必须前置 `timeout` 指令，以防止因脚本逻辑错误（如死循环）导致 Agent 流程无限挂起。
4.  **反馈修正**：若发现逻辑偏离或报告与代码不符，主 Agent 必须打回并要求子 Agent 修正。
...
为了确保代码在不同微处理器架构（如 RH850, ARM, x86）上的物理一致性，并规避 AI 在处理数据宽度时的“幻觉”，本项目禁止直接使用 C 原生类型（如 `int`, `long`, `char`），必须统一使用以下手搓的固定宽度类型：
