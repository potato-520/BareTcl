...
/* 空行：逻辑分段 */
static tcl_u32 tcl_alc_p(TclCtx *context, tcl_u32 byte_count); /* 变量区分配函数前置声明，供下方错误处理辅助函数调用 */
/* 空行：逻辑分段 */
/* 内部字符工具函数：将 ASCII 字母统一转换为小写，便于做大小写不敏感比较 */
static tcl_u8 t_to_lower_ascii(tcl_u8 input_char) { /* 函数入口：执行单字符的小写归一化 */
    if (input_char >= 'A' && input_char <= 'Z') { /* 判定是否命中 ASCII 大写字母区间 */
        return (tcl_u8)(input_char + ('a' - 'A')); /* 命中大写字符时，映射到对应的小写字符 */
    } /* 结束大写字符判定 */
    return input_char; /* 非大写字符保持原值返回 */
} /* 函数执行完毕 */
/* 空行：逻辑分段 */
/* 内部字符串工具函数：大小写不敏感比较，返回语义与 t_scmp 保持一致 */
static tcl_i32 t_sicmp(const tcl_u8 *left_string, const tcl_u8 *right_string) { /* 函数入口：执行 case-insensitive 字符串比较 */
    if (!left_string || !right_string) { /* 判空保护：与 t_scmp 的空指针语义保持一致 */
        return left_string == right_string ? 0 : (left_string ? -1 : 1); /* 返回指针层面的比较结果 */
    } /* 结束判空分支 */
    while (*left_string && *right_string) { /* 双指针线性遍历，逐字符执行不区分大小写比较 */
        tcl_u8 left_char = t_to_lower_ascii(*left_string); /* 归一化左侧字符 */
        tcl_u8 right_char = t_to_lower_ascii(*right_string); /* 归一化右侧字符 */
        if (left_char != right_char) { /* 若归一化后字符不相等，直接返回差值 */
            return (tcl_i32)left_char - (tcl_i32)right_char; /* 语义保持与 t_scmp 一致 */
        } /* 结束字符差异判定 */
        left_string++; /* 推进左侧扫描指针 */
        right_string++; /* 推进右侧扫描指针 */
    } /* 结束逐字符比较循环 */
    return (tcl_i32)t_to_lower_ascii(*left_string) - (tcl_i32)t_to_lower_ascii(*right_string); /* 处理长度不同导致的尾部差异 */
} /* 函数执行完毕 */
/* 空行：逻辑分段 */
/* 错误结果写入辅助函数：将错误文本复制到 Arena 并写入 context->result */
static void tcl_set_error_text(TclCtx *context, const tcl_u8 *error_text) { /* 函数入口：统一封装错误消息写入行为 */
    tcl_u32 error_text_length = t_slen(error_text) + 1; /* 计算错误文本长度并包含结尾终止符 */
    tcl_u32 error_text_offset = tcl_alc_p(context, error_text_length); /* 在 Arena 中申请错误文本的持久化空间 */
    if (error_text_offset == TCL_NULL) { /* 分配失败保护：避免写入非法偏移量 */
        context->result = TCL_NULL; /* 内存不足时降级为无文本错误结果 */
        return; /* 结束函数，避免后续非法访问 */
    } /* 结束分配失败分支 */
    t_mcpy(TO_PTR(context, error_text_offset), error_text, error_text_length); /* 将错误文本完整复制进 Arena */
    context->result = error_text_offset; /* 更新解释器结果寄存器，使 catch 可读取错误文本 */
} /* 函数执行完毕 */
/* 空行：逻辑分段 */
/* 布尔解析辅助函数：对条件表达式结果执行 Tcl 风格的真值解释与非法值校验 */
static tcl_i32 tcl_parse_bool_text(TclCtx *context, const tcl_u8 *bool_text, tcl_i32 *bool_value_ptr) { /* 函数入口：解析并校验布尔文本 */
    if (!bool_text || bool_text[0] == 0) { /* 空字符串在 Tcl 条件语义中属于非法布尔值 */
        tcl_set_error_text(context, (const tcl_u8*)"expected boolean value but got \"\""); /* 写入明确错误信息 */
        return TCL_ERROR; /* 返回错误状态，交由外层状态机执行异常冒泡 */
    } /* 结束空值判定 */
    if (t_is_int(bool_text)) { /* 数值文本分支：支持 0/1 以及其它整数作为布尔值 */
        *bool_value_ptr = (t_atoi(bool_text) != 0) ? 1 : 0; /* 非零为真，零为假 */
        return TCL_OK; /* 数值布尔解析成功 */
    } /* 结束整数分支 */
    if (t_sicmp(bool_text, (const tcl_u8*)"true") == 0 || /* 标准真值关键字 true */
        t_sicmp(bool_text, (const tcl_u8*)"yes") == 0 || /* 标准真值关键字 yes */
        t_sicmp(bool_text, (const tcl_u8*)"on") == 0) { /* 标准真值关键字 on */
        *bool_value_ptr = 1; /* 关键字真值分支：写入逻辑真 */
        return TCL_OK; /* 真值关键字解析成功 */
    } /* 结束真值关键字判定 */
    if (t_sicmp(bool_text, (const tcl_u8*)"false") == 0 || /* 标准假值关键字 false */
        t_sicmp(bool_text, (const tcl_u8*)"no") == 0 || /* 标准假值关键字 no */
        t_sicmp(bool_text, (const tcl_u8*)"off") == 0) { /* 标准假值关键字 off */
        *bool_value_ptr = 0; /* 关键字假值分支：写入逻辑假 */
        return TCL_OK; /* 假值关键字解析成功 */
    } /* 结束假值关键字判定 */
    tcl_u32 bool_text_length = t_slen(bool_text); /* 计算非法布尔文本长度，用于动态拼接错误消息 */
    tcl_u32 prefix_length = t_slen((const tcl_u8*)"expected boolean value but got \""); /* 错误消息前缀长度 */
    tcl_u32 suffix_length = t_slen((const tcl_u8*)"\""); /* 错误消息后缀长度 */
    tcl_u32 message_offset = tcl_alc_p(context, prefix_length + bool_text_length + suffix_length + 1); /* 分配完整错误消息存储空间 */
    if (message_offset == TCL_NULL) { /* 分配失败时退化为固定错误文本，保持错误语义可见 */
        tcl_set_error_text(context, (const tcl_u8*)"expected boolean value"); /* 写入简化错误文本 */
        return TCL_ERROR; /* 返回错误状态码 */
    } /* 结束分配失败分支 */
    tcl_u8 *message_ptr = TO_PTR(context, message_offset); /* 获取错误消息缓冲区物理指针 */
    t_mcpy(message_ptr, (const tcl_u8*)"expected boolean value but got \"", prefix_length); /* 写入消息前缀 */
    t_mcpy(message_ptr + prefix_length, bool_text, bool_text_length); /* 写入触发错误的原始文本 */
    t_mcpy(message_ptr + prefix_length + bool_text_length, (const tcl_u8*)"\"", suffix_length + 1); /* 写入收尾引号和终止符 */
    context->result = message_offset; /* 将动态错误消息回填到解释器结果寄存器 */
    return TCL_ERROR; /* 非法布尔值解析失败，向上抛出错误 */
} /* 函数执行完毕 */
/* 空行：逻辑分段 */
/* 前置声明 */
...
void tcl_gc(TclCtx *context) {      /* 函数入口：启动垃圾回收机制 */
#ifdef TCL_DEBUG
    tcl_hal_puts((const tcl_u8 *)"-- GC 开始 -- p_top=");
    tcl_u8 buf_pt[12];
    t_itoa(context->p_top, buf_pt);
    tcl_hal_puts(buf_pt);
    tcl_hal_puts((const tcl_u8 *)" t_bot=");
    t_itoa(context->t_bot, buf_pt);
    tcl_hal_puts(buf_pt);
    tcl_hal_puts((const tcl_u8 *)"\n");
#else
    TCL_LOG("-- GC 开始 --\n");      /* 输出 GC 启动日志，辅助开发阶段 of 内存水位监控 */
#endif
    ...
    while (frame_offset != TCL_NULL) { /* 沿着执行栈帧链从叶子节点向根节点回溯遍历 */
        ...
        mark_obj(context, frame_ptr->result); /* 标记栈帧级别的临时结果对象 */
...
/* 空行：逻辑分段 */
    ...
    while (frame_offset != TCL_NULL) { /* 循环处理调用链中的每一个活跃栈帧 */
        ...
        UPDATE_PTR(frame_ptr->result); /* 修正栈帧级别结果的偏移量 */
...
#ifdef TCL_DEBUG
    tcl_hal_puts((const tcl_u8 *)"-- GC 结束 -- p_top_after=");
    tcl_u8 buf_pta[12];
    t_itoa(context->p_top, buf_pta);
    tcl_hal_puts(buf_pta);
    tcl_hal_puts((const tcl_u8 *)"\n");
#else
    TCL_LOG("-- GC 结束 --\n");      /* 输出 GC 结束日志，内存池整理完毕 */
#endif
...
static tcl_u32 tcl_str_interp(TclCtx *context, tcl_u32 src_offset, tcl_u32 src_start, tcl_u32 src_len) { /* 字符串变量与转义字符插值函数 */
    tcl_u32 needed_size = 0;                                        /* 初始化计算出的目标字符串物理长度计数器 */
    tcl_u32 read_pos = 0;                                           /* 初始化读取位置游标 */
    tcl_i32 bracket_depth = 0;                                      /* 初始化方括号的嵌套深度计数器 */
    const tcl_u8 *src_str = TO_PTR(context, src_offset) + src_start; /* 获取待插值源字符串物理指针 */
    while (read_pos < src_len) {                                    /* 开始第一轮扫描以计算插值后的总字符长度 */
        tcl_u8 cur_char = src_str[read_pos];                        /* 载入当前位置字符 */
        if (cur_char == '\\' && read_pos + 1 < src_len) {           /* 如果遇到转义斜杠且后续有字符 */
            needed_size++;                                          /* 计算插值后的物理所需大小递增 */
            read_pos += 2;                                          /* 跳过该转义序列 */
        } else if (cur_char == '$') {                               /* 如果遇到变量引导字符美元符 */
            if (bracket_depth > 0) {                                /* 若处于方括号子命令的深层嵌套中 */
                needed_size++;                                      /* $ 视为普通字符直接计入长度 */
                read_pos++;                                         /* 推进源读取游标 */
                continue;                                           /* 跳过变量替换计算 */
            }                                                       /* 结束括号判断 */
            read_pos++;                                             /* 跳过美元引导符 */
            tcl_u8 var_name_buf[64];                                /* 分配临时名字缓冲区 */
            tcl_u32 var_name_len = 0;                               /* 初始化变量名称实际长度计数 */
            while (read_pos < src_len && var_name_len < 63) {       /* 线性扫描提取合法变量名字符 */
                tcl_u8 name_ch = src_str[read_pos];                 /* 获取字符 */
                if ((name_ch >= 'a' && name_ch <= 'z') ||           /* 判断是否为小写字母 */
                    (name_ch >= 'A' && name_ch <= 'Z') ||           /* 判断是否为大写字母 */
                    (name_ch >= '0' && name_ch <= '9') ||           /* 判断是否为数字 */
                    name_ch == '_') {                               /* 判断是否为下划线 */
                    var_name_buf[var_name_len++] = name_ch;         /* 将合法字符拷贝至临时缓冲区 */
                    read_pos++;                                     /* 推进源游标 */
                } else {                                            /* 遇到非法变量名字边界字符 */
                    break;                                          /* 截断跳出名字提取循环 */
                }                                                   /* 结束字符类型判定 */
            }                                                       /* 结束名字提取循环 */
            var_name_buf[var_name_len] = 0;                         /* 写入变量名字临时缓冲区的结束符 */
            if (var_name_len > 0) {                                 /* 如果提取出的变量名字有效 */
                tcl_u32 var_val_offset = tcl_get_var(context, context->curr_f, var_name_buf); /* 动态获取变量偏移 */
                if (var_val_offset != TCL_NULL) {                   /* 如果变量已定义且值不为空 */
                    needed_size += t_slen(TO_PTR(context, var_val_offset)); /* 累加变量值字符串长度 */
                }                                                   /* 结束值有效性判定 */
            } else {                                                /* 若仅有美元符而无合法变量名字 */
                needed_size++;                                      /* 将美元符当作普通符号计入长度 */
            }                                                       /* 结束变量解析分支 */
        } else {                                                    /* 普通字符或者方括号结构 */
            if (cur_char == '[') {                                  /* 识别左方括号 */
                bracket_depth++;                                    /* 子命令方括号深度递增 */
            } else if (cur_char == ']' && bracket_depth > 0) {      /* 识别已匹配的右方括号 */
                bracket_depth--;                                    /* 子命令方括号深度递减 */
            }                                                       /* 结束括号扫描 */
            needed_size++;                                          /* 累加普通字符长度 */
            read_pos++;                                             /* 推进读取游标 */
        }                                                           /* 结束字符分类扫描 */
    }                                                               /* 第一轮扫描循环结束 */
    tcl_u32 out_buf_size = needed_size + 1;                         /* 计算目标占用的总物理大小(含结束符) */
    context->tmp_roots[4] = src_offset;                             /* 在临时根中保护源对象偏移，以防分配内存时触发 GC 导致位移 */
    context->tmp_roots[3] = tcl_alc_p(context, out_buf_size);       /* 从低区 Arena 中分配目标字符串缓冲区空间 */
    if (context->tmp_roots[3] == TCL_NULL) {                       /* 分配空间失败检测 */
        context->tmp_roots[4] = TCL_NULL;                           /* 释放源偏移根保护 */
        return TCL_NULL;                                            /* 返回失败标识 */
    }                                                               /* 结束失败判定 */
    src_offset = context->tmp_roots[4];                             /* 从垃圾回收后的保护根中恢复可能的最新偏移量 */
    context->tmp_roots[4] = TCL_NULL;                               /* 解除源偏移量的垃圾回收临时根保护 */
    tcl_u8 *out_buf = TO_PTR(context, context->tmp_roots[3]);       /* 获取目标物理缓冲区指针 */
    tcl_u32 write_pos = 0;                                          /* 初始化目标写入游标 */
    read_pos = 0;                                                   /* 重置读取游标进行第二轮实际填充扫描 */
    bracket_depth = 0;                                              /* 重置方括号嵌套计数 */
    src_str = TO_PTR(context, src_offset) + src_start;              /* 再次重映射最新的源字符串物理地址（应对 GC compaction 重定位） */
    while (read_pos < src_len && write_pos < out_buf_size - 1) {    /* 线性遍历源字符串执行实际插值与拷贝 */
        tcl_u8 cur_char = src_str[read_pos];                        /* 获取源字符串当前字符 */
        if (cur_char == '\\' && read_pos + 1 < src_len) {           /* 识别并处理转义斜杠结构 */
            read_pos++;                                             /* 跳过转义斜杠 */
            tcl_u8 esc_char = src_str[read_pos];                    /* 获取待转义的物理字符 */
            if (esc_char == 'n') {                                  /* 匹配换行符 \n */
                out_buf[write_pos++] = '\n';                        /* 写入换行符 */
            } else if (esc_char == 't') {                           /* 匹配制表符 \t */
                out_buf[write_pos++] = '\t';                        /* 写入制表符 */
            } else if (esc_char == 'r') {                           /* 匹配回车符 \r */
                out_buf[write_pos++] = '\r';                        /* 写入回车符 */
            } else {                                                /* 其他未定义转义 */
                out_buf[write_pos++] = esc_char;                    /* 原样写入字符 */
            }                                                       /* 结束转义分支 */
            read_pos++;                                             /* 推进读取游标 */
        } else if (cur_char == '$') {                               /* 识别美元符号 */
            if (bracket_depth > 0) {                                /* 若处于方括号子命令的深层嵌套中 */
                out_buf[write_pos++] = '$';                         /* 原样写入美元符号 */
                read_pos++;                                         /* 推进读取游标 */
                continue;                                           /* 结束该字符处理 */
            }                                                       /* 结束括号判断 */
            read_pos++;                                             /* 跳过美元引导符 */
            tcl_u8 var_name_buf[64];                                /* 局部变量名字临时缓冲区 */
            tcl_u32 var_name_len = 0;                               /* 初始化变量名称实际拷贝计数 */
            while (read_pos < src_len && var_name_len < 63) {       /* 线性扫描提取合法变量名字符 */
                tcl_u8 name_ch = src_str[read_pos];                 /* 提取名字中当前字符 */
                if ((name_ch >= 'a' && name_ch <= 'z') ||           /* 判断是否为小写字母 */
                    (name_ch >= 'A' && name_ch <= 'Z') ||           /* 判断是否为大写字母 */
                    (name_ch >= '0' && name_ch <= '9') ||           /* 判断是否为数字 */
                    name_ch == '_') {                               /* 判断是否为下划线 */
                    var_name_buf[var_name_len++] = name_ch;         /* 写入局部名字缓冲区 */
                    read_pos++;                                     /* 推进读取游标 */
                } else {                                            /* 遇到变量名字边界 */
                    break;                                          /* 跳出名字解析 */
                }                                                   /* 结束字符判断 */
            }                                                       /* 结束变量名提取循环 */
            var_name_buf[var_name_len] = 0;                         /* 写入名字缓冲区字符串结束符 */
            if (var_name_len > 0) {                                 /* 若变量名合法有效 */
                tcl_u32 var_val_offset = tcl_get_var(context, context->curr_f, var_name_buf); /* 查找对应变量的值偏移量 */
                if (var_val_offset != TCL_NULL) {                   /* 若变量名已定义并有值 */
                    out_buf = TO_PTR(context, context->tmp_roots[3]); /* 确保分配后重新映射当前写入目标缓冲区物理基地址 */
                    const tcl_u8 *val_str = TO_PTR(context, var_val_offset); /* 获取变量值的物理指针 */
                    tcl_u32 val_copy_len = t_slen(val_str);         /* 计算待复制变量值的字符串长度 */
                    if (write_pos + val_copy_len >= out_buf_size - 1) { /* 写入边界截断安全防范 */
                        val_copy_len = out_buf_size - 1 - write_pos; /* 强制限定为剩余可用安全空间大小 */
                    }                                               /* 结束溢出检测 */
                    t_mcpy(out_buf + write_pos, val_str, val_copy_len); /* 将变量值内容物理复制到插值缓冲区 */
                    write_pos += val_copy_len;                      /* 推进写入游标 */
                }                                                   /* 结束变量存在性判断 */
            } else {                                                /* 若不为合法变量名，仅存在孤立的美元符 */
                out_buf = TO_PTR(context, context->tmp_roots[3]);   /* 重新映射当前物理缓冲区指针 */
                if (write_pos < out_buf_size - 1) {                 /* 安全边界判断 */
                    out_buf[write_pos++] = '$';                     /* 单个美元符当作普通符号写入 */
                }                                                   /* 结束安全检测 */
            }                                                       /* 结束插值处理 */
        } else {                                                    /* 普通字符或者方括号结构的分发 */
            if (cur_char == '[') {                                  /* 识别左方括号 */
                bracket_depth++;                                    /* 嵌套深度递增 */
            } else if (cur_char == ']' && bracket_depth > 0) {      /* 识别右方括号 */
                bracket_depth--;                                    /* 嵌套深度递减 */
            }                                                       /* 结束括号判断 */
            out_buf[write_pos++] = cur_char;                        /* 将普通字符物理写入 */
            read_pos++;                                             /* 推进读取游标 */
        }                                                           /* 结束字符类型处理 */
    }                                                               /* 第二轮扫描循环结束 */
    out_buf = TO_PTR(context, context->tmp_roots[3]);               /* 再次映射当前物理指针以写入字符串终止符 */
    out_buf[write_pos] = 0;                                         /* 封底字符串物理结束标志位 */
    tcl_u32 result_offset = context->tmp_roots[3];                  /* 准备返回插值结果偏移 */
    context->tmp_roots[3] = TCL_NULL;                               /* 解除插值缓存对象的根保护 */
    return result_offset;                                           /* 正常返回插值字符串对象句柄 */
} /* 结束 tcl_str_interp 函数 */
/* 空行：逻辑分段 */
/* 字符串切片克隆辅助函数：从指定源字符串中拷贝 [start, start+len) 子区间到 Arena */
static tcl_u32 tcl_clone_slice(TclCtx *context, tcl_u32 source_offset, tcl_u32 slice_start, tcl_u32 slice_len) { /* 函数入口：克隆字符串切片 */
    context->tmp_roots[15] = source_offset;                                     /* 保护源偏移量，防止分配触发 GC 时被搬迁失效 */
    tcl_u32 clone_offset = tcl_alc_p(context, slice_len + 1);                   /* 为切片内容分配持久化空间（含终止符） */
    if (clone_offset == TCL_NULL) {                                             /* 内存不足时返回失败 */
        context->tmp_roots[15] = TCL_NULL;                                      /* 清空临时根保护区 */
        return TCL_NULL;                                                        /* 向上层返回分配失败 */
    } /* 结束分配失败分支 */
    source_offset = context->tmp_roots[15];                                     /* 从临时保护根获取可能被 GC 重新定位后的最新偏移 */
    context->tmp_roots[15] = TCL_NULL;                                          /* 清空垃圾回收临时根保护位 */
    const tcl_u8 *source_ptr = TO_PTR(context, source_offset);                  /* 获取最新的源字符串物理指针 */
    t_mcpy(TO_PTR(context, clone_offset), source_ptr + slice_start, slice_len); /* 执行切片内容复制 */
    ((tcl_u8*)TO_PTR(context, clone_offset))[slice_len] = 0;                    /* 写入字符串终止符 */
    return clone_offset;                                                        /* 返回切片对象偏移量 */
} /* 函数执行完毕 */
/* 空行：逻辑分段 */
/* 三段拼接辅助函数：将 prefix + middle + suffix 组合为单个字符串对象 */
static tcl_u32 tcl_concat_three(TclCtx *context, tcl_u32 prefix_offset, tcl_u32 middle_offset, tcl_u32 suffix_offset) { /* 函数入口：执行三段字符串拼接 */
    tcl_u32 prefix_len = t_slen(TO_PTR(context, prefix_offset));                /* 计算前缀长度 */
    tcl_u32 middle_len = t_slen(TO_PTR(context, middle_offset));                /* 计算中段长度 */
    tcl_u32 suffix_len = t_slen(TO_PTR(context, suffix_offset));                /* 计算后缀长度 */
    context->tmp_roots[12] = prefix_offset;                                     /* 保护前缀，防止 GC */
    context->tmp_roots[13] = middle_offset;                                     /* 保护中段，防止 GC */
    context->tmp_roots[14] = suffix_offset;                                     /* 保护后缀，防止 GC */
    tcl_u32 merged_offset = tcl_alc_p(context, prefix_len + middle_len + suffix_len + 1); /* 分配最终拼接结果空间 */
    if (merged_offset == TCL_NULL) { /* 分配失败则直接返回错误 */
        context->tmp_roots[12] = context->tmp_roots[13] = context->tmp_roots[14] = TCL_NULL; /* 释放临时保护根 */
        return TCL_NULL; /* 终止拼接流程 */
    } /* 结束分配失败分支 */
    prefix_offset = context->tmp_roots[12];                                     /* 恢复 GC 移动后的前缀最新偏移 */
    middle_offset = context->tmp_roots[13];                                     /* 恢复 GC 移动后的中段最新偏移 */
    suffix_offset = context->tmp_roots[14];                                     /* 恢复 GC 移动后的后缀最新偏移 */
    context->tmp_roots[12] = context->tmp_roots[13] = context->tmp_roots[14] = TCL_NULL; /* 清除拼接对象的根保护位 */
    tcl_u8 *merged_ptr = TO_PTR(context, merged_offset);                        /* 获取目标缓冲区物理指针 */
    t_mcpy(merged_ptr, TO_PTR(context, prefix_offset), prefix_len);             /* 写入前缀部分 */
    t_mcpy(merged_ptr + prefix_len, TO_PTR(context, middle_offset), middle_len); /* 写入中段部分 */
    t_mcpy(merged_ptr + prefix_len + middle_len, TO_PTR(context, suffix_offset), suffix_len); /* 写入后缀部分 */
    merged_ptr[prefix_len + middle_len + suffix_len] = 0;                       /* 写入终止符形成合法 Tcl 字符串 */
    return merged_offset; /* 返回拼接结果偏移量 */
} /* 函数执行完毕 */

/* 表达式规范化辅助函数：为紧邻的运算符补齐空格，避免 "$i+1" 之类 Token 在归约阶段无法拆分 */
/* 设计目的：保持命令替换语义不变，同时让 ST_TOKENIZE 能稳定切分算术/比较运算符 */
static tcl_u32 tcl_expr_normalize(TclCtx *context, tcl_u32 source_offset) {
    const tcl_u8 *source_ptr = TO_PTR(context, source_offset); /* 获取原始表达式物理指针 */
    tcl_u32 source_len = t_slen(source_ptr); /* 获取原始表达式长度 */
    tcl_u32 normalized_capacity = source_len * 3 + 1; /* 预留最坏情况容量：每个字符两侧补空格 */
    context->tmp_roots[1] = source_offset; /* 分配前保护源表达式偏移 */
    tcl_u32 normalized_offset = tcl_alc_p(context, normalized_capacity); /* 分配规范化缓冲区 */
    if (normalized_offset == TCL_NULL) { /* 分配失败判定 */
        context->tmp_roots[1] = TCL_NULL; /* 清理保护根 */
        return TCL_NULL; /* 返回分配失败 */
    } /* 结束分配失败分支 */
    source_ptr = TO_PTR(context, context->tmp_roots[1]); /* 分配后重新加载源表达式物理指针以应对 GC 位移 */
    tcl_u8 *normalized_ptr = TO_PTR(context, normalized_offset); /* 获取目标缓冲区物理指针 */
    tcl_u32 read_index = 0; /* 初始化源读取游标 */
    tcl_u32 write_index = 0; /* 初始化目标写入游标 */
    tcl_u8 previous_non_space = 0; /* 记录最近一个非空白字符，用于判定一元正负号 */
    tcl_i32 bracket_depth = 0; /* 跟踪命令替换方括号深度，避免改写 [] 内部命令参数语义 */
    while (read_index < source_len) { /* 线性扫描整个表达式 */
        tcl_u8 current_char = source_ptr[read_index]; /* 读取当前字符 */
        tcl_u8 next_char = (read_index + 1 < source_len) ? source_ptr[read_index + 1] : 0; /* 读取下一个字符 */
        if (current_char == '[') { /* 命令替换起始符处理 */
            bracket_depth++; /* 进入方括号上下文 */
            normalized_ptr[write_index++] = current_char; /* 原样写入当前字符 */
            previous_non_space = current_char; /* 更新最近非空白字符 */
            read_index++; /* 推进游标 */
            continue; /* 继续下一轮 */
        } /* 结束左方括号处理 */
        if (current_char == ']') { /* 命令替换结束符处理 */
            if (bracket_depth > 0) { /* 防止深度计数下溢 */
                bracket_depth--; /* 退出一层方括号上下文 */
            } /* 结束深度保护 */
            normalized_ptr[write_index++] = current_char; /* 原样写入当前字符 */
            previous_non_space = current_char; /* 更新最近非空白字符 */
            read_index++; /* 推进游标 */
        ...
        if (bracket_depth > 0) { /* 命令替换内部内容不做算符空格改写 */
            normalized_ptr[write_index++] = current_char; /* 保持原始字符 */
            if (!(current_char == ' ' || current_char == '\t' || current_char == '\n' || current_char == '\r')) { /* 仅在非空白时更新最近非空白字符 */
                previous_non_space = current_char; /* 更新最近非空白字符 */
            } /* 结束最近非空白字符更新 */
            read_index++; /* 推进游标 */
            continue; /* 继续下一轮 */
        } /* 结束方括号内部保护 */
        tcl_u32 is_space = (current_char == ' ' || current_char == '\t' || current_char == '\n' || current_char == '\r'); /* 判定空白字符 */
        if (is_space) { /* 空白归一化处理 */
            if (write_index > 0 && normalized_ptr[write_index - 1] != ' ') { /* 若前一个输出字符不是空格则补一个空格 */
                normalized_ptr[write_index++] = ' '; /* 写入单个标准空格 */
            } /* 结束去重写空格 */
            read_index++; /* 推进源游标 */
            continue; /* 继续处理后续字符 */
        } /* 结束空白处理分支 */
        tcl_u32 is_double_operator = 0; /* 标记是否命中双字符运算符 */
        if ((current_char == '<' && next_char == '=') || /* <= */
            ...
            (current_char == '&' && next_char == '&') || /* && */
            (current_char == '|' && next_char == '|')) { /* || */
            is_double_operator = 1; /* 命中双字符运算符 */
        } /* 结束双字符运算符判定 */
        if (is_double_operator) { /* 处理双字符运算符 */
            if (write_index > 0 && normalized_ptr[write_index - 1] != ' ') { /* 运算符前补空格 */
                normalized_ptr[write_index++] = ' '; /* 写入前置空格 */
            } /* 结束前置空格处理 */
            normalized_ptr[write_index++] = current_char; /* 写入运算符第一个字符 */
            normalized_ptr[write_index++] = next_char; /* 写入运算符第二个字符 */
            if (read_index + 2 < source_len && source_ptr[read_index + 2] != ' ') { /* 运算符后补空格，避免粘连下一个 Token */
                normalized_ptr[write_index++] = ' '; /* 写入后置空格 */
            } /* 结束后置空格处理 */
            previous_non_space = next_char; /* 更新最近非空白字符 */
            read_index += 2; /* 跨过双字符运算符 */
            continue; /* 进入下一轮扫描 */
        } /* 结束双字符运算符处理 */
        tcl_u32 is_single_operator = (current_char == '*' || current_char == '/' || current_char == '%' || current_char == '(' || current_char == ')' || current_char == '<' || current_char == '>'); /* 判定单字符运算符 */
        tcl_u32 is_sign_char = (current_char == '+' || current_char == '-'); /* 判定正负号字符 */
        if (is_sign_char) { /* 针对 + / - 做一元符号兼容判定 */
            tcl_u32 is_unary_sign = 0; /* 记录是否属于一元正负号 */
            if (next_char >= '0' && next_char <= '9') { /* 下一字符是数字，可能为一元正负号 */
                if (previous_non_space == 0 || previous_non_space == '(' || previous_non_space == '+' || previous_non_space == '-' || previous_non_space == '*' || previous_non_space == '/' || previous_non_space == '%' || previous_non_space == '<' || previous_non_space == '>' || previous_non_space == '=' || previous_non_space == '!' || previous_non_space == '&' || previous_non_space == '|') { /* 判定当前位于表达式起点或运算符之后 */
                    is_unary_sign = 1; /* 标记为一元正负号 */
                } /* 结束一元判定 */
            } /* 结束数字前瞻判定 */
            if (is_unary_sign) { /* 一元正负号直接拼接到数字上，不补空格 */
                normalized_ptr[write_index++] = current_char; /* 原样写入符号 */
                previous_non_space = current_char; /* 更新最近非空白字符 */
                read_index++; /* 推进游标 */
                continue; /* 继续处理下一字符 */
            } /* 结束一元正负号处理 */
            is_single_operator = 1; /* 非一元时按二元运算符处理 */
        } /* 结束正负号处理 */
        if (is_single_operator) { /* 处理单字符运算符 */
            if (write_index > 0 && normalized_ptr[write_index - 1] != ' ') { /* 运算符前补空格 */
                normalized_ptr[write_index++] = ' '; /* 写入前置空格 */
            } /* 结束前置空格处理 */
            normalized_ptr[write_index++] = current_char; /* 写入运算符 */
            if (read_index + 1 < source_len && source_ptr[read_index + 1] != ' ') { /* 运算符后补空格 */
                normalized_ptr[write_index++] = ' '; /* 写入后置空格 */
            } /* 结束后置空格处理 */
            previous_non_space = current_char; /* 更新最近非空白字符 */
            read_index++; /* 推进游标 */
        ...
        normalized_ptr[write_index++] = current_char; /* 普通字符原样写入 */
        previous_non_space = current_char; /* 更新最近非空白字符 */
        read_index++; /* 推进游标 */
    } /* 结束主扫描循环 */
    if (write_index > 0 && normalized_ptr[write_index - 1] == ' ') { /* 去掉末尾多余空格 */
        write_index--; /* 回退写入位置 */
    } /* 结束尾部空格清理 */
    normalized_ptr[write_index] = 0; /* 写入字符串结束符 */
    context->tmp_roots[1] = TCL_NULL; /* 清理保护根 */
    return normalized_offset; /* 返回规范化表达式偏移 */
} /* 结束 tcl_expr_normalize 函数 */
...
static tcl_i32 tcl_cmd_set(TclCtx *context, tcl_i32 argument_count, tcl_u32 *argument_values) {
    ...
    if (argument_count == 2) {       /* 场景 1：仅提供变量名，执行读取操作 */
        ...
        if (variable_value_offset == TCL_NULL) { /* 检查变量是否存在 */
            tcl_u32 variable_name_offset = argument_values[1]; /* 记录变量名偏移量，用于拼接可诊断的错误文本 */
            const tcl_u8 *variable_name_ptr = TO_PTR(context, variable_name_offset); /* 在分配前缓存变量名指针 */
            tcl_u32 variable_name_length = t_slen(variable_name_ptr); /* 计算变量名长度 */
            tcl_u32 prefix_length = t_slen((const tcl_u8*)"can't read \""); /* 计算错误消息前缀长度 */
            tcl_u32 middle_length = t_slen((const tcl_u8*)"\": no such variable"); /* 计算错误消息尾部长度 */
            context->tmp_roots[15] = variable_name_offset; /* 临时保护变量名对象，防止分配错误文本时触发 GC 导致偏移失效 */
            tcl_u32 error_text_offset = tcl_alc_p(context, prefix_length + variable_name_length + middle_length + 1); /* 分配完整错误文本存储区 */
            if (error_text_offset != TCL_NULL) { /* 分配成功分支：构造详细错误信息 */
                variable_name_ptr = TO_PTR(context, context->tmp_roots[15]); /* 分配后重载变量名指针，兼容 GC 地址搬迁 */
                tcl_u8 *error_text_ptr = TO_PTR(context, error_text_offset); /* 获取错误文本缓冲区指针 */
                t_mcpy(error_text_ptr, (const tcl_u8*)"can't read \"", prefix_length); /* 写入前缀部分 */
                t_mcpy(error_text_ptr + prefix_length, variable_name_ptr, variable_name_length); /* 写入变量名内容 */
                t_mcpy(error_text_ptr + prefix_length + variable_name_length, (const tcl_u8*)"\": no such variable", middle_length + 1); /* 写入尾部并包含字符串终止符 */
                context->result = error_text_offset; /* 将错误文本挂载到 result，供 catch 第二参数读取 */
            } else { /* 分配失败分支：退化为固定错误文本，避免无消息失败 */
                tcl_set_error_text(context, (const tcl_u8*)"no such variable"); /* 写入简化错误信息 */
            } /* 结束错误文本构造分支 */
            context->tmp_roots[15] = TCL_NULL; /* 清理临时 GC 根保护位 */
...
static tcl_i32 tcl_cmd_if(TclCtx *context, tcl_i32 argument_count, tcl_u32 *argument_values) {
    ...
    tcl_u32 condition_script_offset = argument_values[1]; /* 条件脚本偏移：默认直接使用原始参数 */
    if (cond_ptr[0] == '{') {
        ...
        condition_script_offset = stripped; /* 记录剥离后的脚本偏移 */
    } /* 结束外层花括号剥离 */
    tcl_u32 normalized_condition_offset = tcl_expr_normalize(context, condition_script_offset); /* 对条件表达式运算符做规范化，支持无空格写法 */
    if (normalized_condition_offset == TCL_NULL) { /* 规范化失败判定 */
        return TCL_ERROR; /* 返回错误 */
    } /* 结束规范化失败处理 */
    active_frame_ptr->cond = normalized_condition_offset; /* 存储规范化后的条件脚本 */
...
/* 辅助函数：负责对无括号的扁平子表达式区间进行四轮优先级归约计算 */
static tcl_u32 tcl_reduce_flat_expr(TclCtx *context, tcl_u32 *base_argv, tcl_u32 *base_argc_ptr, tcl_u32 sub_start, tcl_u32 sub_len) {
    if (sub_len == 0) { /* 边界判定：子区间为空 */
        return TCL_OK; /* 直接返回成功 */
    } /* 结束边界判定 */
    if (sub_len == 1) { /* 边界判定：仅单个操作数，无需规约 */
        return TCL_OK; /* 直接返回成功 */
    } /* 结束边界判定 */
    tcl_i32 round_idx = 3; /* 初始化轮次变量为 3，代表从 Level 3 (最高优先级) 开始向下进行归约 */
    while (round_idx >= 0) { /* 循环进行四轮优先级扫描，依次处理 Level 3, 2, 1, 0 */
        tcl_u32 cursor = sub_start + 1; /* 运算符起始探测点，限定在子区间内 */
        while (cursor < sub_start + sub_len - 1) { /* 循环遍历该子区间内所有剩余元素 */
            const tcl_u8 *operator_str = TO_PTR(context, base_argv[cursor]); /* 获取运算符指针 */
            ...
            if (is_match) { /* 匹配到运算符 */
                tcl_i32 left_operand_val = t_atoi(TO_PTR(context, base_argv[cursor - 1])); /* 获取左操作数 */
                tcl_i32 right_operand_val = t_atoi(TO_PTR(context, base_argv[cursor + 1])); /* 获取右操作数 */
                tcl_u8 *left_ptr = TO_PTR(context, base_argv[cursor - 1]); /* 获取左操作数物理指针 */
                tcl_u8 *right_ptr = TO_PTR(context, base_argv[cursor + 1]); /* 获取右操作数物理指针 */
                ...
                /* GC 安全分配防护模块 */
                TclFrame *frame_ptr = TO_PTR(context, context->curr_f); /* 获取当前帧物理指针 */
                tcl_u32 total_argc = *base_argc_ptr; /* 取出当前全局数组的总计数 */
                for (tcl_u32 copy_idx = 0; copy_idx < total_argc; copy_idx++) { /* 备份本地数组到物理帧 */
                    frame_ptr->argv[copy_idx] = base_argv[copy_idx]; /* 逐个拷入 */
                } /* 结束备份 */
                frame_ptr->argc = total_argc; /* 更新物理帧参数数量计数 */
                result_offset = tcl_alc_p(context, 12); /* 调用内存分配器申请 12 字节空间 */
                if (result_offset == TCL_NULL) { /* 分配失败判定 */
                    context->status = TCL_ERROR; /* 设置状态为错误 */
                    return TCL_ERROR; /* 报错退出 */
                } /* 结束分配判定 */
                frame_ptr = TO_PTR(context, context->curr_f); /* 重新加载 GC 移动后的当前帧物理指针 */
                for (tcl_u32 copy_idx = 0; copy_idx < total_argc; copy_idx++) { /* 从物理帧恢复本地数组 */
                    base_argv[copy_idx] = frame_ptr->argv[copy_idx]; /* 拷回本地数组，此时所有偏移已被 GC 安全修正 */
                } /* 结束恢复 */
                t_itoa(calc_result, TO_PTR(context, result_offset)); /* 将十进制格式化结果写入新分配的物理空间 */
                base_argv[cursor - 1] = result_offset; /* 更新左侧操作数为计算结果的偏移量 */
                for (tcl_u32 move_idx = cursor + 2; move_idx < total_argc; move_idx++) { /* 将右侧运算后的后续元素前移 */
                    base_argv[move_idx - 2] = base_argv[move_idx]; /* 覆盖前移 */
                } /* 结束前移 */
                *base_argc_ptr -= 2; /* 总计数修正减 2 */
                sub_len -= 2; /* 当前子区间长度缩减减 2 */
            } else { /* 运算符不匹配，跳过当前项 */
                cursor += 2; /* 步进 2 探测下一个运算符 */
            } /* 结束匹配判定 */
        } /* 结束内层元素遍历 */
        round_idx--; /* 进入下一优先级轮次 */
    } /* 结束四轮归约 */
    return TCL_OK; /* 求值正常，返回成功 */
}
...
static tcl_i32 tcl_cmd_expr(TclCtx *context, tcl_i32 argument_count, tcl_u32 *argument_values) {
    ...
    /* 设计：expr 的参数无论是否经过花括号处理，都需要：
       1. 对 $var 进行变量替换（tcl_str_interp），并保留 [cmd] 的延迟执行语义
       2. 对算术/比较运算符进行空格规范化，保证 "$i+1" 也可被正确分词
       3. 将规范化后的表达式送入新帧（FRAME_IS_EXPR）执行归约求值 */
    if (argument_count == 2) { /* 单参数模式：剥括号后需要展开并在新帧求值 */
        ...
        const tcl_u8 *inner_expr_ptr = TO_PTR(context, inner_offset); /* 获取去括号后的表达式物理指针 */
        tcl_u32 inner_expr_len = t_slen(inner_expr_ptr); /* 获取去括号后的表达式长度 */
        context->tmp_roots[0] = inner_offset; /* 保护表达式对象防止被 GC 回收 */
        tcl_u32 interpolated_offset = tcl_str_interp(context, context->tmp_roots[0], 0, inner_expr_len); /* 先做变量插值并保留方括号延迟语义 */
        context->tmp_roots[0] = TCL_NULL; /* 清理插值临时保护根 */
        if (interpolated_offset == TCL_NULL) { /* 插值失败判定 */
            return TCL_ERROR; /* 返回错误 */
        } /* 结束插值失败处理 */
        tcl_u32 normalized_offset = tcl_expr_normalize(context, interpolated_offset); /* 对表达式运算符做空格规范化 */
        if (normalized_offset == TCL_NULL) { /* 规范化失败判定 */
            return TCL_ERROR; /* 返回错误 */
        } /* 结束规范化失败处理 */
        /* 在 Arena 栈区分配新帧用于执行规范化后的表达式 */
        context->tmp_roots[0] = normalized_offset; /* 挂载根保护，防止分配帧触发 GC 导致表达式对象死亡 */
        ...
        inner_offset = context->tmp_roots[0]; /* 重新加载可能因 GC 位移的对象偏移量（此时为规范化表达式） */
...
/* 列表长度计算辅助函数：复用 llength 的线性扫描语义，为 end/end±N 索引解析提供长度基准 */
static tcl_i32 tcl_calc_list_len(const tcl_u8 *list_text_ptr) { /* 函数入口：计算列表文本中的元素总数 */
    tcl_i32 element_count = 0; /* 初始化元素计数器 */
    tcl_i32 brace_depth = 0; /* 初始化花括号嵌套深度寄存器 */
    const tcl_u8 *scan_ptr = list_text_ptr; /* 初始化线性扫描指针 */
    while (scan_ptr && *scan_ptr) { /* 主循环：扫描整个列表文本 */
        while (*scan_ptr == ' ' || *scan_ptr == '\t' || *scan_ptr == '\n') { /* 跳过元素前导空白 */
            scan_ptr++; /* 推进扫描指针 */
        } /* 结束前导空白跳过 */
        if (!*scan_ptr) { /* 扫描到字符串末尾时终止 */
            break; /* 退出主循环 */
        } /* 结束末尾判定 */
        element_count++; /* 命中一个新元素，计数器加一 */
        if (*scan_ptr == '{') { /* 命中花括号元素时进入嵌套匹配路径 */
            scan_ptr++; /* 跳过起始左花括号 */
            brace_depth = 1; /* 初始化嵌套深度为 1 */
            while (*scan_ptr && brace_depth > 0) { /* 扫描直到配对的右花括号闭合 */
                if (*scan_ptr == '{') { /* 识别内层左花括号 */
                    brace_depth++; /* 深度递增 */
                } else if (*scan_ptr == '}') { /* 识别右花括号 */
                    brace_depth--; /* 深度递减 */
                } /* 结束花括号判定 */
                scan_ptr++; /* 推进扫描游标 */
            } /* 结束花括号元素扫描 */
        } else { /* 普通原子元素分支 */
            while (*scan_ptr && *scan_ptr != ' ' && *scan_ptr != '\t' && *scan_ptr != '\n') { /* 扫描至下一个空白边界 */
                scan_ptr++; /* 推进扫描游标 */
            } /* 结束原子元素扫描 */
        } /* 结束元素类型分支 */
    } /* 结束主循环 */
    return element_count; /* 返回最终元素数量 */
} /* 函数执行完毕 */
/* 空行：逻辑分段 */
/* 列表索引解析辅助函数：支持数值索引以及 end/end±N 语法 */
static tcl_i32 tcl_parse_list_index(const tcl_u8 *index_text_ptr, tcl_i32 list_length) { /* 函数入口：将索引文本转换为具体数值索引 */
    if (t_scmp(index_text_ptr, (const tcl_u8*)"end") == 0) { /* 处理 end 关键字 */
        return list_length - 1; /* 返回最后一个元素的索引值 */
    } /* 结束 end 判定 */
    if (index_text_ptr[0] == 'e' && index_text_ptr[1] == 'n' && index_text_ptr[2] == 'd' && index_text_ptr[3] == '-') { /* 处理 end-N 语法 */
        const tcl_u8 *delta_text_ptr = index_text_ptr + 4; /* 取出偏移量文本部分 */
        if (t_is_int(delta_text_ptr)) { /* 偏移量必须是合法整数 */
            return (list_length - 1) - t_atoi(delta_text_ptr); /* 返回 end 向左偏移 N 的索引 */
        } /* 结束偏移量合法性判定 */
    } /* 结束 end-N 判定 */
    if (index_text_ptr[0] == 'e' && index_text_ptr[1] == 'n' && index_text_ptr[2] == 'd' && index_text_ptr[3] == '+') { /* 处理 end+N 语法 */
        const tcl_u8 *delta_text_ptr = index_text_ptr + 4; /* 取出偏移量文本部分 */
        if (t_is_int(delta_text_ptr)) { /* 偏移量必须是合法整数 */
            return (list_length - 1) + t_atoi(delta_text_ptr); /* 返回 end 向右偏移 N 的索引 */
        } /* 结束偏移量合法性判定 */
    } /* 结束 end+N 判定 */
    return t_atoi(index_text_ptr); /* 其它情况退化为普通整数索引解析 */
} /* 函数执行完毕 */
/* 空行：逻辑分段 */
...
static tcl_i32 tcl_cmd_lindex(TclCtx *context, tcl_i32 argument_count, tcl_u32 *argument_values) {
    ...
    tcl_i32 list_length = tcl_calc_list_len(list_scan_ptr); /* 先计算列表长度，为 end/end±N 解析提供基准 */
    tcl_i32 target_index = tcl_parse_list_index(TO_PTR(context, argument_values[2]), list_length); /* 解析用户指定的目标索引语法 */
    ...
    while (*list_scan_ptr) { /* 线性扫描整个列表字符串 */
        ...
        if (current_element_idx == target_index) { /* 检查当前扫描到的元素是否命中目标索引 */
            tcl_u32 element_start_idx = element_start_pos - (const tcl_u8 *)TO_PTR(context, argument_values[1]); /* 计算当前元素起始物理位置相对原列表的逻辑偏移量 */
            ...
            const tcl_u8 *new_element_start_pos = TO_PTR(context, argument_values[1]) + element_start_idx; /* 重新载入 GC 后的物理指针 */
            t_mcpy(TO_PTR(context, element_clone_offset), new_element_start_pos, element_physical_len); /* 执行内容物理拷贝 */
...
static tcl_i32 tcl_cmd_lrange(TclCtx *context, tcl_i32 argument_count, tcl_u32 *argument_values) {
    ...
    const tcl_u8 *source_list_text_ptr = TO_PTR(context, argument_values[1]); /* 缓存源列表文本指针，供索引解析与拷贝阶段复用 */
    tcl_i32 list_length = tcl_calc_list_len(source_list_text_ptr); /* 计算源列表长度，支持 end/end±N 索引语义 */
    tcl_i32 first_idx = tcl_parse_list_index(TO_PTR(context, argument_values[2]), list_length); /* 解析并转换起始索引 */
    tcl_i32 last_idx = tcl_parse_list_index(TO_PTR(context, argument_values[3]), list_length); /* 解析并转换结束索引（支持 end-1/end-2） */
    if (last_idx < first_idx) { /* 当结束索引落在起始索引左侧时，按 Tcl 语义返回空列表 */
        tcl_u32 empty_list_offset = tcl_alc_p(context, 1); /* 分配仅包含终止符的空字符串对象 */
        if (empty_list_offset == TCL_NULL) { /* 分配失败则返回内存错误 */
            return TCL_ERROR; /* 向上层抛出错误状态 */
        } /* 结束分配失败判定 */
        ((tcl_u8*)TO_PTR(context, empty_list_offset))[0] = 0; /* 写入空字符串终止符 */
        context->result = empty_list_offset; /* 设置空列表为当前返回结果 */
        return TCL_OK; /* 正常返回 */
    } /* 结束空区间判定 */
    /* 分配结果缓冲区的内存空间，为防止越界，采用保守策略分配与原列表等长的物理空间 */
    tcl_u32 result_buffer_offset = tcl_alc_p(context, t_slen(source_list_text_ptr) + 1); /* 分配结果区 */
    if (result_buffer_offset == TCL_NULL) { /* 检查分配结果 */
        return TCL_ERROR; /* 空间不足时终止执行 */
    } /* 结束结果区分配检查 */
    source_list_text_ptr = TO_PTR(context, argument_values[1]); /* 关键：GC 后重新载入源列表物理指针 */
    tcl_u8 *write_cursor_ptr = TO_PTR(context, result_buffer_offset); /* 定位结果列表缓冲区的写入游标 */
    const tcl_u8 *source_list_ptr = source_list_text_ptr; /* 定位源列表的读取游标 */
    ...
    while (*source_list_ptr) { /* 开始对源列表进行线性遍历解析 */
        ...
        if (current_item_count >= first_idx && (last_idx == tcl_i32_MAX || current_item_count <= last_idx)) { /* 区间判定 */
            ...
            tcl_i32 copy_len = source_list_ptr - item_content_start - ((brace_depth == 0 && item_content_start > source_list_text_ptr && *(item_content_start - 1) == '{') ? 1 : 0); /* 计算拷贝长度 */
...
#ifndef TCL_YIELD_HOOK
#define TCL_YIELD_HOOK()
#endif
        TCL_YIELD_HOOK();
        ...
        switch (frame->state) { /* 基于当前帧内部状态寄存器的值，进行逻辑分支跳转 */
            case ST_TOKENIZE: { /* 分词状态：该阶段负责从当前的 pc 位置开始，识别出命令名称、参数及注释，处理物理分隔符 */
                ...
                while (script_ptr[frame->pc] && frame->argc < MAX_ARGS) { /* 设定 MAX_ARGS 阈值以防止 argv 数组越界 */
                    ...
                    if (script_ptr[frame->pc] == '{') { /* 特殊语法：处理由花括号包裹的“字面量” Token（保留外括号至展开阶段剥离） */
                        ...
                        tcl_i32 brace_depth = 0; /* 初始化花括号深度，确保 {...} 内部的 [ ] 不参与外层命令替换配对 */
                        frame->pc++; /* 跨过左方括号引导符 */
                        start_pos = frame->pc - 1; /* 记录包含引导方括号在内的完整脚本片段起始点 */
                        while (script_ptr[frame->pc] && depth > 0) { /* 搜索逻辑：寻找配对的右方括号以界定子脚本边界 */
                            if (script_ptr[frame->pc] == '\\' && script_ptr[frame->pc + 1]) { /* 处理转义字符：跳过被转义的后继字节 */
                                frame->pc += 2; /* 一次性越过反斜杠和其后继字符 */
                                continue; /* 继续下一轮扫描 */
                            } /* 结束转义处理 */
                            if (script_ptr[frame->pc] == '{') { /* 识别花括号起点 */
                                brace_depth++; /* 进入或加深花括号字面量上下文 */
                            } else if (script_ptr[frame->pc] == '}' && brace_depth > 0) { /* 识别花括号终点 */
                                brace_depth--; /* 退出一层花括号字面量上下文 */
                            } else if (brace_depth == 0 && script_ptr[frame->pc] == '[') { /* 仅在非花括号上下文中统计子命令嵌套 */
                                depth++; /* 增加方括号嵌套深度 */
                            } else if (brace_depth == 0 && script_ptr[frame->pc] == ']') { /* 仅在非花括号上下文中识别闭合方括号 */
                                depth--; /* 减少方括号嵌套深度 */
                            } /* 结束方括号嵌套判定 */
                            frame->pc++; /* 游标推进，跨过当前方括号或内部指令字符 */
                        } /* 结束配对方括号搜索循环 */
                        if (depth != 0) { /* 语法错误判定：扫描至脚本末尾仍未匹配到闭合右方括号 */
                            tcl_set_error_text(context, (const tcl_u8*)"missing close-bracket"); /* 写入与标准 Tcl 对齐的诊断文本 */
                            context->status = TCL_ERROR; /* 标记解析阶段错误状态 */
                            frame->argc = 0; /* 清空参数寄存器，避免后续继续执行残缺命令 */
                            frame->exp_idx = 0; /* 重置展开索引 */
                            frame->state = ST_RESUME; /* 切换到恢复态，复用统一异常冒泡路径 */
                            goto next_state_loop; /* 立即重启调度循环，让错误信号向上层 catch 冒泡 */
                        } /* 结束缺失右方括号判定 */
            ...
            case ST_EXPAND: { /* 指令展开阶段：负责处理参数数组中的动态内容，如 $变量 和 [子命令] */
                while (frame->exp_idx < frame->argc) { /* 线性迭代处理 argv 中的每一个 Token */
                    ...
                    if (arg_ptr[0] == '$') { /* 变量替换：检测到以美元符号引导的 Token */
                        ...
                        if (dq_len > 1) {
                            tcl_u32 interp = tcl_str_interp(context, frame->argv[frame->exp_idx], 1, dq_len - 2);
                            frame = TO_PTR(context, context->curr_f);
                            if (interp != TCL_NULL) {
                                frame->argv[frame->exp_idx] = interp;
                                frame->state = ST_EXPAND_STR; /* 双引号参数改走专用状态，支持内部 [cmd] 命令替换 */
                                goto next_state_loop; /* 立即切换状态机，避免误增 exp_idx 丢失当前参数上下文 */
                            }
                        }
            ...
            case ST_EXPAND_STR: { /* 双引号字符串展开态：对已完成 $ 与转义处理的字符串执行 [cmd] 命令替换 */
                if (frame->exp_idx >= frame->argc) { /* 防御判定：若参数索引越界则回到常规展开流程 */
                    frame->state = ST_EXPAND; /* 恢复到通用参数展开状态 */
                    break; /* 退出当前状态处理 */
                } /* 结束越界判定 */
                tcl_u32 string_offset = frame->argv[frame->exp_idx]; /* 获取当前正在处理的双引号参数对象 */
                tcl_u8 *string_ptr = TO_PTR(context, string_offset); /* 解析为物理字符串指针 */
                tcl_u32 string_len = t_slen(string_ptr); /* 计算字符串总长度 */
                tcl_u32 bracket_start = TCL_NULL; /* 记录第一个待替换子命令的左方括号位置 */
                tcl_u32 bracket_end = TCL_NULL; /* 记录匹配的右方括号位置 */
                tcl_i32 bracket_depth = 0; /* 子命令嵌套深度计数器 */
                for (tcl_u32 scan_index = 0; scan_index < string_len; scan_index++) { /* 线性扫描字符串寻找第一个完整 [cmd] 片段 */
                    if (string_ptr[scan_index] == '[') { /* 识别左方括号 */
                        if (bracket_depth == 0) { /* 仅记录最外层子命令起点 */
                            bracket_start = scan_index; /* 保存最外层左括号位置 */
                        } /* 结束起点记录 */
                        bracket_depth++; /* 嵌套深度递增 */
                    } else if (string_ptr[scan_index] == ']') { /* 识别右方括号 */
                        if (bracket_depth > 0) { /* 仅在存在未闭合左括号时处理 */
                            bracket_depth--; /* 嵌套深度递减 */
                            if (bracket_depth == 0 && bracket_start != TCL_NULL) { /* 命中最外层子命令闭合点 */
                                bracket_end = scan_index; /* 记录匹配右括号位置 */
                                break; /* 找到第一个完整片段即可进入替换流程 */
                            } /* 结束闭合点判定 */
                        } /* 结束右括号深度保护 */
                    } /* 结束括号字符判定 */
                } /* 结束扫描循环 */
                if (bracket_start == TCL_NULL) { /* 无任何 [cmd] 片段时，该参数已完成全部字符串替换 */
                    frame->exp_idx++; /* 推进到下一个参数 */
                    frame->state = ST_EXPAND; /* 回到通用参数展开态 */
                    break; /* 退出双引号展开态 */
                } /* 结束无子命令分支 */
                if (bracket_end == TCL_NULL) { /* 仅发现左括号但未匹配到右括号，属于语法错误 */
                    tcl_set_error_text(context, (const tcl_u8*)"missing close-bracket"); /* 写入诊断信息 */
                    context->status = TCL_ERROR; /* 标记错误状态 */
                    frame->state = ST_RESUME; /* 进入恢复态触发统一异常冒泡 */
                    break; /* 退出当前状态处理 */
                } /* 结束缺失闭合括号判定 */
                tcl_u32 prefix_len = bracket_start; /* 计算前缀长度（左括号之前的文本） */
                tcl_u32 command_len = bracket_end - bracket_start + 1; /* 计算 [cmd] 片段总长度（含方括号） */
                tcl_u32 suffix_len = string_len - bracket_end - 1; /* 计算后缀长度（右括号之后的文本） */
                context->tmp_roots[9] = string_offset; /* 保护原字符串对象偏移，防止后续分配触发 GC 时失效 */
                tcl_u32 prefix_offset = tcl_clone_slice(context, context->tmp_roots[9], 0, prefix_len); /* 克隆前缀文本 */
                if (prefix_offset == TCL_NULL) { /* 分配失败保护 */
                    context->tmp_roots[9] = TCL_NULL; /* 释放保护根 */
                    context->status = TCL_ERROR; /* 标记内存错误 */
                    frame->state = ST_RESUME; /* 走统一异常冒泡 */
                    break; /* 退出 */
                } /* 结束前缀克隆判定 */
                context->tmp_roots[10] = prefix_offset; /* 在根集合中保护前缀对象 */

                tcl_u32 command_offset = tcl_clone_slice(context, context->tmp_roots[9], bracket_start, command_len); /* 克隆 [cmd] 片段 */
                if (command_offset == TCL_NULL) { /* 分配失败保护 */
                    context->tmp_roots[9] = context->tmp_roots[10] = TCL_NULL; /* 释放保护根 */
                    context->status = TCL_ERROR; /* 标记内存错误 */
                    frame->state = ST_RESUME; /* 走统一异常冒泡 */
                    break; /* 退出 */
                } /* 结束子命令克隆判定 */
                context->tmp_roots[11] = command_offset; /* 在根集合中保护子命令对象 */

                tcl_u32 suffix_offset = tcl_clone_slice(context, context->tmp_roots[9], bracket_end + 1, suffix_len); /* 克隆后缀文本 */
                context->tmp_roots[9] = TCL_NULL; /* 克隆结束，释放原字符串保护根 */
                if (suffix_offset == TCL_NULL) { /* 分配失败保护 */
                    context->tmp_roots[10] = context->tmp_roots[11] = TCL_NULL; /* 释放保护根 */
                    context->status = TCL_ERROR; /* 标记内存错误 */
                    frame->state = ST_RESUME; /* 走统一异常冒泡 */
                    break; /* 退出 */
                } /* 结束后缀克隆判定 */
                context->tmp_roots[12] = suffix_offset; /* 在根集合中保护后缀对象 */

                tcl_u32 new_frame_offset = tcl_alc_t(context, sizeof(TclFrame)); /* 为 [cmd] 子命令分配执行栈帧 */
                if (new_frame_offset == TCL_NULL) { /* 栈帧分配失败保护 */
                    context->tmp_roots[10] = context->tmp_roots[11] = context->tmp_roots[12] = TCL_NULL; /* 释放保护根 */
                    context->status = TCL_ERROR; /* 标记内存错误 */
                    frame->state = ST_RESUME; /* 走统一异常冒泡 */
                    break; /* 退出 */
                } /* 结束子帧分配判定 */

                /* 从保护根恢复可能发生位移的对象逻辑偏移量 */
                prefix_offset = context->tmp_roots[10]; /* 从临时根恢复前缀文本偏移量 */
                command_offset = context->tmp_roots[11]; /* 从临时根恢复命令片段偏移量 */
                suffix_offset = context->tmp_roots[12]; /* 从临时根恢复后缀文本偏移量 */

                tcl_u8 *command_ptr = TO_PTR(context, command_offset); /* 获取 [cmd] 片段字符串指针 */
                tcl_u32 command_text_len = t_slen(command_ptr); /* 计算 [cmd] 字符串长度 */
                tcl_u32 command_script_offset = tcl_alc_p(context, command_text_len); /* 分配剥离方括号后的脚本存储空间 */
                if (command_script_offset == TCL_NULL) { /* 内存不足判定 */
                    context->tmp_roots[10] = context->tmp_roots[11] = context->tmp_roots[12] = TCL_NULL; /* 释放保护根 */
                    context->status = TCL_ERROR; /* 标记内存错误 */
                    frame->state = ST_RESUME; /* 走统一异常冒泡 */
                    break; /* 退出 */
                ...
                /* 再次恢复可能发生位移的对象逻辑偏移量 */
                prefix_offset = context->tmp_roots[10];
                command_offset = context->tmp_roots[11];
                suffix_offset = context->tmp_roots[12];

                /* 清空临时保护根，此时所有对象即将被挂载至栈帧中 */
                context->tmp_roots[10] = context->tmp_roots[11] = context->tmp_roots[12] = TCL_NULL;

                command_ptr = TO_PTR(context, command_offset); /* 重新加载命令片段指针，防止 GC 触发后失效 */
                t_mcpy(TO_PTR(context, command_script_offset), command_ptr + 1, command_text_len - 2); /* 拷贝方括号内部脚本正文 */
                ((tcl_u8*)TO_PTR(context, command_script_offset))[command_text_len - 2] = 0; /* 写入脚本终止符 */
                TclFrame *sub_frame = TO_PTR(context, new_frame_offset); /* 映射子命令执行帧 */
                sub_frame->script = command_script_offset; /* 挂载子命令脚本 */
                sub_frame->pc = 0; /* 子命令从脚本开头执行 */
                sub_frame->vars = TCL_NULL; /* 子帧不建立私有变量表，使用共享作用域策略 */
                sub_frame->parent = context->curr_f; /* 建立父子调用链返回路径 */
                sub_frame->scope = context->curr_f; /* 子命令共享父帧作用域，支持访问当前局部变量 */
                sub_frame->state = ST_TOKENIZE; /* 子帧初始进入分词态 */
                sub_frame->flags = FRAME_SHARE_SCOPE; /* 标记为共享作用域执行帧 */
                sub_frame->cond = sub_frame->body = sub_frame->result = TCL_NULL; /* 初始化控制槽位 */
                sub_frame->argc = sub_frame->exp_idx = 0; /* 初始化参数计数寄存器 */
                frame->cond = prefix_offset; /* 暂存前缀字符串，供子命令返回后拼接使用 */
                frame->body = suffix_offset; /* 暂存后缀字符串，供子命令返回后拼接使用 */
                frame->result = command_offset; /* 暂存命令片段对象偏移，便于调试与后续定位 */
                frame->state = ST_EXPAND_STR_RESUME; /* 挂起父帧至字符串恢复态，等待子命令求值结果 */
                context->curr_f = new_frame_offset; /* 切换执行焦点到子命令执行帧 */
                goto next_state_loop; /* 立即驱动子帧执行 */
            } /* 结束 ST_EXPAND_STR 状态块 */
            case ST_EXPAND_STR_RESUME: { /* 双引号字符串恢复态：将 [cmd] 返回值拼接回原字符串并继续扫描后续片段 */
                if (context->status != TCL_OK) { /* 子命令求值失败时，直接交由统一恢复态做异常冒泡 */
                    frame->state = ST_RESUME; /* 状态迁移到通用恢复态 */
                    break; /* 退出当前状态处理 */
                } /* 结束错误状态判定 */
                context->tmp_roots[9] = frame->cond; /* 保护前缀偏移，防止拼接分配期间触发 GC 失效 */
                context->tmp_roots[10] = context->result; /* 保护子命令返回值偏移 */
                context->tmp_roots[11] = frame->body; /* 保护后缀偏移 */
                tcl_u32 merged_offset = tcl_concat_three(context, context->tmp_roots[9], context->tmp_roots[10], context->tmp_roots[11]); /* 拼接 prefix + cmdResult + suffix */
                context->tmp_roots[9] = TCL_NULL; /* 释放保护根 9 */
                context->tmp_roots[10] = TCL_NULL; /* 释放保护根 10 */
                context->tmp_roots[11] = TCL_NULL; /* 释放保护根 11 */
                if (merged_offset == TCL_NULL) { /* 拼接失败时按内存错误处理 */
                    context->status = TCL_ERROR; /* 标记内存错误 */
                    frame->state = ST_RESUME; /* 交给通用恢复态冒泡 */
                    break; /* 退出 */
                } /* 结束拼接失败判定 */
                frame->argv[frame->exp_idx] = merged_offset; /* 回写当前参数为拼接后的新字符串 */
                frame->cond = frame->body = frame->result = TCL_NULL; /* 清理字符串替换临时槽位 */
                frame->state = ST_EXPAND_STR; /* 继续扫描当前参数，处理可能存在的下一个 [cmd] */
                break; /* 退出恢复态处理 */
            } /* 结束 ST_EXPAND_STR_RESUME 状态块 */
            case ST_EXECUTE: { /* 执行阶段：根据命令名称分发至 C 内建函数或脚本定义的过程 (Proc) */
                ...
                if (!found) { /* 若在 C 内建指令集中未命中，则尝试在全局作用域寻找脚本定义的 Proc */
                    ...
                    if (body_offset != TCL_NULL) { /* 若已识别到有效的脚本过程定义 */
                        context->tmp_roots[0] = args_offset; /* 提前保护形参列表偏移，防止分配新栈帧触发 GC 时丢失重定位 */
                        context->tmp_roots[1] = body_offset; /* 提前保护过程体偏移，防止分配新栈帧触发 GC 时丢失重定位 */
                        tcl_u32 new_frame_offset = tcl_alc_t(context, sizeof(TclFrame)); /* 为本次过程调用分配独立的执行栈帧 */
                        tcl_u32 parent_frame_offset = context->curr_f; /* 提前记录父栈帧偏移量，供参数绑定及指针刷新使用 */
                        args_offset = context->tmp_roots[0]; /* 从根保护重新加载经 GC 修正后的形参列表偏移 */
                        body_offset = context->tmp_roots[1]; /* 从根保护重新加载经 GC 修正后的过程体偏移 */
                        if (new_frame_offset == TCL_NULL) { /* 栈内存安全性校验 */
                            context->status = TCL_ERROR; /* 栈空间不足，记录错误状态 */
                            context->tmp_roots[0] = TCL_NULL; /* 失败时清理临时保护根 0 */
                            context->tmp_roots[1] = TCL_NULL; /* 失败时清理临时保护根 1 */
                        } else { /* 实例化过程执行环境 */
                            ...
                            new_frame->parent = parent_frame_offset; /* 建立物理调用链回溯，指向当前的调用者帧 */
                            ...
                            context->curr_f = new_frame_offset; /* 提前将当前活跃帧切换为新栈帧，使其进入 GC 追溯树 */
                            context->tmp_roots[1] = TCL_NULL; /* 过程体脚本已绑定，解除临时保护根 1 */
                            frame = TO_PTR(context, parent_frame_offset); /* 刷新父栈帧的物理指针，以防分配新栈帧时发生 GC 位移 */
                            /* 空行：逻辑分段 - 形参与实参绑定逻辑 */
                            ...
                            while (arg_idx < frame->argc) { /* 循环处理调用方传入的每一个有效实参 */
                                ...
                                frame = TO_PTR(context, parent_frame_offset); /* 刷新父栈帧物理指针，以防分配形参副本时 GC 造成位移 */
                                ...
                                if (token_len == 4 && t_scmp(TO_PTR(context, context->tmp_roots[1]), (const tcl_u8*)"args") == 0) { /* 如果形参名是 args 关键字 */
                                    ...
                                    frame = TO_PTR(context, parent_frame_offset); /* 刷新父栈帧物理指针以应对分配导致的 GC 搬移 */
                                ...
                                frame = TO_PTR(context, parent_frame_offset); /* 再次刷新物理指针以备下次迭代使用 */
                            } /* 结束所有形参的绑定循环 */
                            context->tmp_roots[0] = TCL_NULL; /* 过程环境建立闭环，解除形参列表字符串保护 */
                            frame->state = ST_RESUME; /* 调用者帧进入挂起恢复态，静待过程执行完成后的结果冒泡 */
                            /* 注意：context->curr_f 已经在循环前成功切换至新帧，此处无需重复切换 */
                        ...
                        tcl_hal_puts((const tcl_u8*)"'\nTokens:\n");
                        for (tcl_i32 arg_index = 0; arg_index < frame->argc; arg_index++) {
                            tcl_hal_puts((const tcl_u8*)"  [");
                            t_itoa(arg_index, argc_buf); tcl_hal_puts(argc_buf);
                            tcl_hal_puts((const tcl_u8*)"]: '");
                            if (frame->argv[arg_index] != TCL_NULL) tcl_hal_puts(TO_PTR(context, frame->argv[arg_index]));
                            tcl_hal_puts((const tcl_u8*)"'\n");
                        }
            ...
            case ST_EXPR_REDUCE: { /* 表达式归约阶段：负责对已展开的 Token 进行逻辑或算术缩减，而非作为指令执行 */
                ...
                if (frame->argc == 0) { /* 边界情况：无 Token */
                    ...
                    tcl_u32 temp_argv[MAX_ARGS]; /* 防御性本地拷贝工作区数组，避免破坏原栈帧数据 */
                    tcl_u32 temp_argc = 0; /* 初始化本地参数数量计数器 */
                    context->tmp_roots[12] = tcl_alc_p(context, 2); /* 分配左括号字符串的Arena物理空间，用临时根12保护以防GC */
                    if (context->tmp_roots[12] == TCL_NULL) { /* 分配左括号物理空间失败判定 */
                        context->status = TCL_ERROR; /* 设置解释器状态为错误 */
                        return TCL_ERROR; /* 报错退出 */
                    } /* 结束判定 */
                    t_mcpy(TO_PTR(context, context->tmp_roots[12]), "(", 2); /* 物理复制左括号字符 */

                    context->tmp_roots[13] = tcl_alc_p(context, 2); /* 分配右括号字符串的Arena物理空间，用临时根13保护以防GC */
                    if (context->tmp_roots[13] == TCL_NULL) { /* 分配右括号物理空间失败判定 */
                        context->tmp_roots[12] = TCL_NULL; /* 级联释放左括号的垃圾回收保护 */
                        context->status = TCL_ERROR; /* 设置解释器状态为错误 */
                        return TCL_ERROR; /* 报错退出 */
                    } /* 结束判定 */
                    t_mcpy(TO_PTR(context, context->tmp_roots[13]), ")", 2); /* 物理复制右括号字符 */
                    frame = TO_PTR(context, context->curr_f); /* 刷新当前执行栈帧物理指针以防垃圾回收导致失效 */

                    for (tcl_u32 sync_index = 0; sync_index < frame->argc; sync_index++) { /* 过滤并清洗所有表达式参数 */
                        tcl_u8 *value_pointer = TO_PTR(context, frame->argv[sync_index]); /* 获取当前参数的物理指针 */
                        if (value_pointer == 0) { /* 防御性保护：遇到空参数偏移量直接视为表达式非法 */
                            goto expr_error; /* 统一走错误出口，避免空指针解引用 */
                        } /* 结束空指针保护 */
                        tcl_u32 value_length = t_slen(value_pointer); /* 获取当前参数的物理长度 */

                        tcl_u32 leading_parentheses = 0; /* 初始化前导左括号计数 */
                        while (leading_parentheses < value_length && value_pointer[leading_parentheses] == '(') { /* 统计前导连续左括号数量 */
                            leading_parentheses++; /* 前导左括号计数器累加 */
                        } /* 结束前导统计 */

                        tcl_u32 trailing_parentheses = 0; /* 初始化尾部右括号计数 */
                        while (trailing_parentheses < value_length - leading_parentheses && value_pointer[value_length - 1 - trailing_parentheses] == ')') { /* 统计尾部连续右括号数量 */
                            trailing_parentheses++; /* 尾部右括号计数器累加 */
                        } /* 结束尾部统计 */

                        if (value_length > 0 && leading_parentheses + trailing_parentheses == value_length) { /* 纯括号组成的Token判定，必须保证长度大于0以防止空字符串被误过滤 */
                            for (tcl_u32 character_index = 0; character_index < value_length; character_index++) { /* 拆分纯括号并拷入工作区 */
                                tcl_u32 parenthesis_offset = (value_pointer[character_index] == '(') ? context->tmp_roots[12] : context->tmp_roots[13]; /* 按字符选择左右括号偏移量 */
                                if (temp_argc < MAX_ARGS) { /* 本地工作区界限溢出保护 */
                                    temp_argv[temp_argc++] = parenthesis_offset; /* 存入本地参数数组 */
                                } /* 结束保护 */
                            } /* 结束拆分拷入 */
                        } else { /* 操作数或变量中包含粘连括号 */
                            for (tcl_u32 paren_index = 0; paren_index < leading_parentheses; paren_index++) { /* 依次填入前导左括号 */
                                if (temp_argc < MAX_ARGS) { /* 数组溢出界限保护 */
                                    temp_argv[temp_argc++] = context->tmp_roots[12]; /* 存入左括号偏移量 */
                                } /* 结束保护 */
                            } /* 结束前导拷入 */

                            tcl_u32 token_offset = frame->argv[sync_index]; /* 默认使用原始偏移量 */
                            tcl_u8 *clean_pointer = value_pointer + leading_parentheses; /* 干净操作数的起始物理地址 */
                            tcl_u32 clean_length = value_length - leading_parentheses - trailing_parentheses; /* 计算干净操作数长度 */

                            if (clean_pointer[0] == '$') { /* 变量引用判定 */
                                tcl_u8 saved_character = value_pointer[leading_parentheses + clean_length]; /* 备份干净部分后面的首个字符 */
                                value_pointer[leading_parentheses + clean_length] = 0; /* 临时写入物理截断符 */
                                tcl_u32 variable_value_offset = tcl_get_var(context, context->curr_f, clean_pointer + 1); /* 获取变量对应的值 */
                                value_pointer[leading_parentheses + clean_length] = saved_character; /* 还原被备份的物理字符 */
                                if (variable_value_offset == TCL_NULL) { /* 未定义变量引用非法判定 */
                                    goto expr_error; /* 跳转至错误出口进行统一异常处理，防止直接返回跳过冒泡流程 */
                                } /* 结束判定 */
                                token_offset = variable_value_offset; /* 就地替换为变量值偏移 */
                            } else if (clean_length < value_length) { /* 常数或常规字面量粘连括号 */
                                TclFrame *frame_pointer = TO_PTR(context, context->curr_f); /* 获取物理执行栈帧 */
                                for (tcl_u32 copy_index = 0; copy_index < temp_argc; copy_index++) { /* 同步本地工作区以防GC重定位 */
                                    frame_pointer->argv[copy_index] = temp_argv[copy_index]; /* 写回物理栈帧 */
                                } /* 结束同步 */
                                for (tcl_u32 null_index = temp_argc; null_index < sync_index; null_index++) { /* 清理空闲区域 */
                                    frame_pointer->argv[null_index] = TCL_NULL; /* 置为空以防GC Trace野指针 */
                                } /* 结束清理 */
                                frame_pointer->argc = frame->argc; /* 保持原参数个数不变 */

                                tcl_u32 new_string_offset = tcl_alc_p(context, clean_length + 1); /* 分配干净常数字符串的Arena物理空间 */
                                if (new_string_offset == TCL_NULL) { /* 物理内存耗尽判定 */
                                    goto expr_error; /* 跳转至错误出口进行统一异常处理，防止直接返回跳过冒泡流程 */
                                } /* 结束判定 */

                                frame = TO_PTR(context, context->curr_f); /* 刷新因分配GC引起的物理栈帧指针位移 */
                                for (tcl_u32 copy_index = 0; copy_index < temp_argc; copy_index++) { /* 恢复本地工作区 */
                                    temp_argv[copy_index] = frame->argv[copy_index]; /* 拷回本地 */
                                } /* 结束恢复 */

                                value_pointer = TO_PTR(context, frame->argv[sync_index]); /* 刷新源物理字符指针 */
                                clean_pointer = value_pointer + leading_parentheses; /* 重新计算干净部分首地址 */
                                t_mcpy(TO_PTR(context, new_string_offset), clean_pointer, clean_length); /* 物理拷贝干净字符串 */
                                ((tcl_u8*)TO_PTR(context, new_string_offset))[clean_length] = 0; /* 封底写入终止符 */
                                token_offset = new_string_offset; /* 换用新分配的GC安全逻辑偏移 */
                            } /* 结束字面量拷贝判定 */

                            if (temp_argc < MAX_ARGS) { /* 溢出界限保护 */
                                temp_argv[temp_argc++] = token_offset; /* 拷入操作数 */
                            } /* 结束保护 */

                            for (tcl_u32 paren_index = 0; paren_index < trailing_parentheses; paren_index++) { /* 依次拷入尾部右括号 */
                                if (temp_argc < MAX_ARGS) { /* 溢出界限保护 */
                                    temp_argv[temp_argc++] = context->tmp_roots[13]; /* 拷入右括号 */
                                } /* 结束保护 */
                            } /* 结束尾部拷入 */
                        } /* 结束粘连括号分支处理 */
                    } /* 结束参数清洗遍历 */

                    /* 3. 使用最内层优先策略循环消除所有的括号对 */
                    while (1) { /* 开启死循环直到括号全部消除完毕 */
                        tcl_i32 right_parenthesis_idx = -1; /* 初始化右括号索引 */
                        for (tcl_u32 index = 0; index < temp_argc; index++) { /* 从左往右寻找第一个右括号 */
                            tcl_u8 *val_ptr = TO_PTR(context, temp_argv[index]); /* 获取当前 token 物理指针 */
                            if (t_scmp(val_ptr, (tcl_u8*)")") == 0) { /* 匹配右括号 */
                                right_parenthesis_idx = (tcl_i32)index; /* 记录右括号位置 */
                                break; /* 跳出查找循环 */
                            } /* 结束匹配判定 */
                        } /* 结束右括号查找 */

                        if (right_parenthesis_idx == -1) { /* 若数组中不存在右括号 */
                            break; /* 说明所有括号对已处理完毕，退出消除循环 */
                        } /* 结束判定 */

                        tcl_i32 left_parenthesis_idx = -1; /* 初始化左括号索引 */
                        for (tcl_i32 index = right_parenthesis_idx - 1; index >= 0; index--) { /* 从右括号位置往左搜索最近的左括号 */
                            tcl_u8 *val_ptr = TO_PTR(context, temp_argv[index]); /* 获取 token 物理指针 */
                            if (t_scmp(val_ptr, (tcl_u8*)"(") == 0) { /* 匹配左括号 */
                                left_parenthesis_idx = index; /* 记录左括号位置 */
                                break; /* 跳出查找循环 */
                            } /* 结束匹配判定 */
                        } /* 结束左括号查找 */

                        if (left_parenthesis_idx == -1) { /* 若有右括号但左侧不存在匹配的左括号 */
                            goto expr_error; /* 跳转至错误出口进行统一异常处理，防止直接返回跳过冒泡流程 */
                        } /* 结束判定 */

                        tcl_u32 sub_start = left_parenthesis_idx + 1; /* 计算括号内表达式的起始索引 */
                        tcl_u32 sub_len = right_parenthesis_idx - left_parenthesis_idx - 1; /* 计算括号内表达式的长度 */
                        if (sub_len == 0) { /* 空括号对判定，如 "()" */
                            goto expr_error; /* 跳转至错误出口进行统一异常处理，防止直接返回跳过冒泡流程 */
                        } /* 结束判定 */

                        /* 调用辅助函数，对最内层括号内部的扁平子表达式进行归约 */
                        if (tcl_reduce_flat_expr(context, temp_argv, &temp_argc, sub_start, sub_len) != TCL_OK) { /* 归约失败判定 */
                            goto expr_error; /* 跳转至错误出口进行统一异常处理，防止直接返回跳过冒泡流程 */
                        } /* 结束判定 */

                        /* 消除圆括号对：将规约得到的计算结果（已放在 sub_start，即 left_parenthesis_idx + 1）覆盖左括号 */
                        temp_argv[left_parenthesis_idx] = temp_argv[left_parenthesis_idx + 1]; /* 覆盖左括号 */

                        /* 将原本在右括号（此时在 left_parenthesis_idx + 2）后面的所有元素整体前移 2 位覆盖右括号和空余槽位 */
                        for (tcl_u32 move_idx = left_parenthesis_idx + 3; move_idx < temp_argc; move_idx++) { /* 遍历右括号后继元素 */
                            temp_argv[move_idx - 2] = temp_argv[move_idx]; /* 覆盖前移 */
                        } /* 结束平移 */
                        temp_argc -= 2; /* 总计数修正减 2 */
                    } /* 结束消除循环 */

                    /* 检查并验证是否仍然残留有未匹配的左括号 '(' */
                    for (tcl_u32 index = 0; index < temp_argc; index++) { /* 遍历数组中剩余的 Token */
                        tcl_u8 *val_ptr = TO_PTR(context, temp_argv[index]); /* 获取 token 物理指针 */
                        if (t_scmp(val_ptr, (tcl_u8*)"(") == 0) { /* 发现残留左括号 */
                            goto expr_error; /* 跳转至错误出口进行统一异常处理，防止直接返回跳过冒泡流程 */
                        } /* 结束判定 */
                    } /* 结束左括号残余校验 */

                    /* 4. 对消除括号后的最终扁平表达式数组调用辅助函数进行总体归约 */
                    if (tcl_reduce_flat_expr(context, temp_argv, &temp_argc, 0, temp_argc) != TCL_OK) { /* 最终归约失败判定 */
                        goto expr_error; /* 跳转至错误出口进行统一异常处理，防止直接返回跳过冒泡流程 */
                    } /* 结束判定 */
                    ...
                    context->tmp_roots[12] = TCL_NULL; /* 释放左括号临时根保护，保持内存管理完整 */
                    context->tmp_roots[13] = TCL_NULL; /* 释放右括号临时根保护，保持内存管理完整 */
                }

                context->status = TCL_OK; /* 归约指令集语义完成，强制将全局状态重置为成功运行态 */
                frame = TO_PTR(context, context->curr_f); /* 重新加载当前帧物理指针，以防垃圾回收导致偏移失效 */
                frame->state = ST_TOKENIZE; /* 逻辑闭环：设为 TOKENIZE 态以触发 ST_RESUME 阶段的栈帧自动清理与结果冒泡 */
                break; /* 退出 ST_EXPR_REDUCE 状态处理 */
            expr_error:
                context->tmp_roots[12] = TCL_NULL; /* 释放左括号临时根保护，执行统一清理 */
                context->tmp_roots[13] = TCL_NULL; /* 释放右括号临时根保护，执行统一清理 */
                context->status = TCL_ERROR; /* 设置全局错误状态码，以配合外层 catch 机制 */
                { /* 开启异常冒泡流程，释放当前表达式帧并回退到父级调用上下文 */
                    tcl_i32 current_status = context->status; /* 暂存当前的异常状态码 */
                    tcl_u32 current_result = context->result; /* 暂存相关的计算结果或错误提示信息 */
                    while (frame) { /* 异常冒泡流程：沿栈帧调用链向上寻找最近的能够拦截或消费当前异常状态的拦截点 */
                        tcl_u8 current_flags = frame->flags; /* 获取当前扫描栈帧的功能标志属性 */
                        tcl_u32 parent_offset = frame->parent; /* 暂存父栈帧逻辑地址以供后续跳跃 */
                        context->t_bot += ((sizeof(TclFrame) + 7) & ~7); /* 物理回收当前已被证明失败的子帧空间，并维持8字节对齐 */
                        context->curr_f = parent_offset; /* 更新解释器中当前最顶层活跃帧的地址 */
                        if (parent_offset == TCL_NULL) { /* 已彻底冒泡到顶层外部调用者结界，没有被任何拦截器捕获 */
                            frame = 0; /* 标志活跃帧归零 */
                            break; /* 终止冒泡流程 */
                        } /* 结束顶层判定 */
                        frame = TO_PTR(context, parent_offset); /* 物理重新定位新父帧的指针以进行下一轮决策 */
                        if (frame->state == ST_CATCH_END || ((current_status == TCL_BREAK || current_status == TCL_CONTINUE) && (frame->state == ST_LOOP || frame->state == ST_COND))) { /* 判断是否命中拦截条件 */
                            break; /* 命中拦截目标，在此终结冒泡并挂载到当前帧继续处理 */
                        } /* 结束拦截判定 */
                        if (current_status == TCL_RETURN && (current_flags & FRAME_IS_PROC)) { /* 判断是否到达函数返回值传递结界 */
                            break; /* 到达过程作用域，停止冒泡并将状态码转换为返回值传递 */
                        } /* 结束过程边界判定 */
                    } /* 结束层级回溯循环 */
                    context->status = current_status; /* 恢复最终被拦截后或顶层溢出后的信号状态码 */
                    context->result = current_result; /* 同步恢复对应的计算结果或报错消息 */
                    if (!frame) context->curr_f = TCL_NULL; /* 穿透全局顶层则进入系统停机安全态 */
                } /* 结束异常冒泡处理块 */
                break; /* 退出 case，走大循环进行下一轮活跃帧调度 */
            ...
            case ST_IF_BODY: { /* 分支指令：判定条件布尔结果并决策是否执行主体 (body) */
                ...
                tcl_i32 bool_condition_value = 0; /* 定义布尔值寄存器，承接条件文本解析结果 */
                if (tcl_parse_bool_text(context, result_string, &bool_condition_value) != TCL_OK) { /* 先按 Tcl 规则解析条件真值，非法值直接报错 */
                    context->status = TCL_ERROR; /* 标记为条件语义错误，交由外层异常冒泡处理 */
                    break; /* 中断当前 ST_IF_BODY 流程，避免继续执行分支脚本 */
                } /* 结束条件布尔解析判定 */
                if (bool_condition_value) { /* 条件解析为逻辑真，进入 if 主体执行路径 */
                    ...
                    if (frame->result != TCL_NULL) { /* 存在 else body，需要执行（else_body 存在 frame->result 中） */
                        context->tmp_roots[0] = frame->result; /* 保护 else 脚本偏移量，防止分配子帧触发 GC 时丢失重定位 */
                        ...
                        if (else_frame_offset == TCL_NULL) { /* 内存安全性校验 */
                            context->status = TCL_ERROR; /* 报错 */
                            context->tmp_roots[0] = TCL_NULL; /* 出错时清理保护根 */
                            context->curr_f = TCL_NULL; /* 终止 */
                            break; /* 退出 */
                        } /* 结束分配校验 */
                        frame = TO_PTR(context, context->curr_f); /* 分配后重新同步 if 帧指针，防止 GC 位移 */
                        tcl_u32 else_body_script = context->tmp_roots[0]; /* 从临时根加载经 GC 修正后的有效偏移量 */
                        context->tmp_roots[0] = TCL_NULL; /* 任务完成，立即释放保护根 */
            ...
            case ST_LOOP: { /* 循环执行阶段：根据条件探测结果，决策是否进入或继续执行 while 循环体 */
                ...
                tcl_i32 bool_condition_value = 0; /* 定义布尔值寄存器，承接 while 条件解析结果 */
                if (tcl_parse_bool_text(context, result_string, &bool_condition_value) != TCL_OK) { /* 先执行 Tcl 规则布尔解析，非法值立即中断循环 */
                    context->status = TCL_ERROR; /* 标记非法布尔表达式错误 */
                    break; /* 交由外层异常冒泡处理 */
                } /* 结束 while 条件布尔解析判定 */
                if (bool_condition_value) { /* 条件解析为逻辑真，进入循环体执行 */
...
static tcl_u32 tcl_collect_all_command_names(TclCtx *context) {  /* 函数入口：生成 info commands 的完整列表 */
    tcl_u32 total_length = 1;  /* 预留字符串终止符 */
    tcl_i32 has_written_name = 0;  /* 标记结果中是否已写入过命令名 */
    for (tcl_i32 command_index = 0; command_index < cmd_count; command_index++) {  /* 遍历 C 原子指令表 */
        const tcl_u8 *command_name = cmd_table[command_index].name;  /* 读取命令名称 */
        if (has_written_name) {  /* 若前面已经写入过一个命令名 */
            total_length += 1;  /* 补一个空格分隔符 */
        }  /* 结束分隔符判定 */
        total_length += t_slen(command_name);  /* 累加命令名字符长度 */
        has_written_name = 1;  /* 标记结果中已有至少一个命令名 */
    }  /* 结束 C 指令长度统计 */
    tcl_u32 global_offset = context->g_vars;  /* 从全局变量表开始遍历 Tcl proc */
    while (global_offset != TCL_NULL) {  /* 逐项扫描全局符号链表 */
        TclVar *global_var = TO_PTR(context, global_offset);  /* 获取变量节点 */
        const tcl_u8 *global_name = TO_PTR(context, global_var->name);  /* 获取变量名字符串 */
        if (global_name[0] == 'p' && global_name[1] == ':') {  /* 仅收集 p: 前缀的过程名 */
            if (has_written_name) {  /* 命中已有输出内容时 */
                total_length += 1;  /* 在新命令前追加空格 */
            }  /* 结束分隔符判定 */
            total_length += t_slen(global_name + 2);  /* 仅统计 p: 后面的过程名 */
            has_written_name = 1;  /* 标记结果中已有命令名 */
        }  /* 结束 proc 名称判定 */
        global_offset = global_var->next;  /* 前进到下一个全局变量节点 */
    }  /* 结束 proc 长度统计 */
    tcl_u32 list_offset = tcl_alc_p(context, total_length);  /* 为完整命令列表分配空间 */
    if (list_offset == TCL_NULL) {  /* 分配失败 */
        return TCL_NULL;  /* 向上层报告失败 */
    }  /* 结束分配失败判定 */
    tcl_u8 *write_ptr = TO_PTR(context, list_offset);  /* 重新定位写入指针 */
    has_written_name = 0;  /* 第二遍写回时重新计数 */
    for (tcl_i32 command_index = 0; command_index < cmd_count; command_index++) {  /* 写回 C 指令名 */
        const tcl_u8 *command_name = cmd_table[command_index].name;  /* 获取命令名 */
        if (has_written_name) {  /* 不是第一个元素时 */
            *write_ptr++ = ' ';  /* 写入空格分隔符 */
        }  /* 结束分隔符判定 */
        tcl_u32 command_name_length = t_slen(command_name);  /* 计算命令名长度 */
        t_mcpy(write_ptr, command_name, command_name_length);  /* 写入命令名内容 */
        write_ptr += command_name_length;  /* 推进写指针 */
        has_written_name = 1;  /* 标记已有输出 */
    }  /* 结束 C 指令写回 */
    global_offset = context->g_vars;  /* 重新从全局变量表头开始遍历 proc */
    while (global_offset != TCL_NULL) {  /* 逐项写回 proc 名称 */
        TclVar *global_var = TO_PTR(context, global_offset);  /* 获取变量节点 */
        const tcl_u8 *global_name = TO_PTR(context, global_var->name);  /* 获取变量名字符串 */
        if (global_name[0] == 'p' && global_name[1] == ':') {  /* 仅写回 proc 条目 */
            if (has_written_name) {  /* 非首项时 */
                *write_ptr++ = ' ';  /* 写入空格分隔符 */
            }  /* 结束分隔符判定 */
            tcl_u32 proc_name_length = t_slen(global_name + 2);  /* 计算 proc 名称长度 */
            t_mcpy(write_ptr, global_name + 2, proc_name_length);  /* 写入 proc 名称 */
            write_ptr += proc_name_length;  /* 推进写指针 */
            has_written_name = 1;  /* 标记已有输出 */
        }  /* 结束 proc 条目判定 */
        global_offset = global_var->next;  /* 前进到下一个变量节点 */
    }  /* 结束 proc 写回 */
    *write_ptr = 0;  /* 写入终止符 */
    return list_offset;  /* 返回命令列表对象 */
}  /* 结束 info commands 辅助函数 */
...
static tcl_i32 tcl_cmd_info(TclCtx *context, tcl_i32 argument_count, tcl_u32 *argument_values) {  /* 函数定义入口：标准的 C 命令实现原型 */
    ...
    if (t_scmp(sub_command_str, (const tcl_u8 *)"commands") == 0) {  /* 子指令分发：处理 info commands 分支请求 */
        if (argument_count > 2) {  /* 逻辑判定：如果用户带有特定的查询过滤模式串 */
            ...
            tcl_u32 empty_list_offset = tcl_alc_p(context, 1);  /* 分配真实空字符串对象 */
            if (empty_list_offset == TCL_NULL) {  /* 分配失败 */
                return TCL_ERROR;  /* 向上层报告内存错误 */
            }  /* 结束分配失败判定 */
            ((tcl_u8*)TO_PTR(context, empty_list_offset))[0] = 0;  /* 写入终止符 */
            context->result = empty_list_offset;  /* 查询失败时返回显式空字符串 */
        } else {  /* 场景分支：处理 info commands 的无参数调用形式（获取所有列表） */
            context->result = tcl_collect_all_command_names(context);  /* 返回完整命令列表 */
            if (context->result == TCL_NULL) {  /* 分配失败时 */
                return TCL_ERROR;  /* 向上层报告内存错误 */
            }  /* 结束分配失败判定 */
        }  /* 结束模式过滤逻辑判定 */
...
void tcl_init(void *arena_ptr, tcl_i32 total_size) {  /* 函数入口：接收由宿主环境预分配的连续物理内存块首地址 */
    ...
    tcl_register_c_cmd((tcl_u8*)"lrange", tcl_cmd_lrange);  /* 列表指令注册：按索引区间提取子列表指令 lrange（含 end±N 语义） */
    tcl_register_c_cmd((tcl_u8*)"unset", tcl_cmd_unset);  /* 变量指令注册：销毁指定变量指令 unset */
    tcl_register_c_cmd((tcl_u8*)"info", tcl_cmd_info);  /* 运行时自省指令注册：info commands 支持无参列举与精确匹配 */
    /* 注意：global 维持 tcllib.tcl 自举实现；lrange 为补齐 end±N 语义在 C 层保留注册 */
