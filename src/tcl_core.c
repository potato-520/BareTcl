                        ...
                        if (new_frame_offset == TCL_NULL) { /* 栈内存安全性校验 */
                            ...
                            while (arg_idx < frame->argc) { /* 循环处理调用方传入的每一个有效实参 */
                                ...
                                if (token_len == 4 && t_scmp(TO_PTR(context, context->tmp_roots[1]), (const tcl_u8*)"args") == 0) { /* 如果形参名是 args 关键字 */
                                    ...
                                    for (tcl_i32 check_idx = arg_idx; check_idx < frame->argc; check_idx++) { /* 遍历后续的实参计数 */
                                        list_len += t_slen(TO_PTR(context, frame->argv[check_idx])); /* 累加当前实参长度 */
                                    } /* 结束长度累加 */
                                    ...
                                    for (tcl_i32 write_idx = arg_idx; write_idx < frame->argc; write_idx++) { /* 循环执行实参拷贝拼接 */
                                        tcl_u8 *src_ptr = TO_PTR(context, frame->argv[write_idx]); /* 提取源实参物理指针 */
                        ...
                        while (cursor < temp_argc - 1) { /* 循环遍历所有剩余元素 */
                            ...
                            if (is_match) { /* 匹配到运算符 */
                                ...
                                if (t_scmp(operator_str, (tcl_u8*)"*") == 0) { /* 乘 */
                                    ...
                                    calc_result = (t_scmp(TO_PTR(context, temp_argv[cursor - 1]), TO_PTR(context, temp_argv[cursor + 1])) == 0); /* 纯字面量对比 */
                                } else if (t_scmp(operator_str, (tcl_u8*)"ne") == 0) { /* 字面量不等 */
                                    calc_result = (t_scmp(TO_PTR(context, temp_argv[cursor - 1]), TO_PTR(context, temp_argv[cursor + 1])) != 0); /* 纯字面量对比 */
                                } else if (t_scmp(operator_str, (tcl_u8*)"==") == 0) { /* 相等 */
                                    tcl_u32 str_equal = (t_scmp(TO_PTR(context, temp_argv[cursor - 1]), TO_PTR(context, temp_argv[cursor + 1])) == 0); /* 优先字面量对比 */
                                    if (str_equal) { /* 相同 */
                                        calc_result = 1; /* 设为真 */
                                    } else { /* 不同 */
                                    ...
                                    tcl_u32 str_not_equal = (t_scmp(TO_PTR(context, temp_argv[cursor - 1]), TO_PTR(context, temp_argv[cursor + 1])) != 0); /* 优先字面量对比 */
                                    if (!str_not_equal) { /* 相同 */
                                        calc_result = 0; /* 结果为假 */
                                    } else { /* 不同 */
                                        calc_result = (left_operand_val != right_operand_val); /* 数值比较 */
