...
static tcl_u32 tcl_str_interp(TclCtx *context, tcl_u32 src_offset, tcl_u32 src_start, tcl_u32 src_len) {
    tcl_u32 needed_size = 0;
    tcl_u32 read_pos = 0;
    tcl_i32 bracket_depth = 0;
    const tcl_u8 *src_str = TO_PTR(context, src_offset) + src_start;
    while (read_pos < src_len) {
        tcl_u8 cur_char = src_str[read_pos];
        if (cur_char == '\\' && read_pos + 1 < src_len) {
            needed_size++;
            read_pos += 2;
        } else if (cur_char == '$') {
            if (bracket_depth > 0) {
            ...
            read_pos++;
            tcl_u8 var_name_buf[64];
            tcl_u32 var_name_len = 0;
            while (read_pos < src_len && var_name_len < 63) {
                tcl_u8 name_ch = src_str[read_pos];
                if ((name_ch >= 'a' && name_ch <= 'z') ||
                    (name_ch >= 'A' && name_ch <= 'Z') ||
                    (name_ch >= '0' && name_ch <= '9') ||
                    name_ch == '_') {
                    var_name_buf[var_name_len++] = name_ch;
                    read_pos++;
                } else {
                    break;
                }
            }
            var_name_buf[var_name_len] = 0;
            if (var_name_len > 0) {
                tcl_u32 var_val_offset = tcl_get_var(context, context->curr_f, var_name_buf);
                if (var_val_offset != TCL_NULL) {
                    needed_size += t_slen(TO_PTR(context, var_val_offset));
                }
            } else {
                needed_size++;
            }
            ...
            if (cur_char == '[') {
                bracket_depth++;
            } else if (cur_char == ']' && bracket_depth > 0) {
                bracket_depth--;
            }
            needed_size++;
            read_pos++;
    ...
    tcl_u32 out_buf_size = needed_size + 1;
    context->tmp_roots[4] = src_offset; /* 保护源对象偏移，防止分配触发 GC 失效 */
    context->tmp_roots[3] = tcl_alc_p(context, out_buf_size);
    if (context->tmp_roots[3] == TCL_NULL) {
        context->tmp_roots[4] = TCL_NULL;
        return TCL_NULL;
    }
    src_offset = context->tmp_roots[4];
    context->tmp_roots[4] = TCL_NULL;
    tcl_u8 *out_buf = TO_PTR(context, context->tmp_roots[3]);
    tcl_u32 write_pos = 0;
    read_pos = 0;
    bracket_depth = 0;
    src_str = TO_PTR(context, src_offset) + src_start; /* GC 后重载源字符串物理指针 */
    while (read_pos < src_len && write_pos < out_buf_size - 1) {
        tcl_u8 cur_char = src_str[read_pos];
        if (cur_char == '\\' && read_pos + 1 < src_len) {
            read_pos++;
            tcl_u8 esc_char = src_str[read_pos];
            if (esc_char == 'n') {
                out_buf[write_pos++] = '\n';
            } else if (esc_char == 't') {
                out_buf[write_pos++] = '\t';
            ...
            if (var_name_len > 0) {
                tcl_u32 var_val_offset = tcl_get_var(context, context->curr_f, var_name_buf);
                if (var_val_offset != TCL_NULL) {
                    out_buf = TO_PTR(context, context->tmp_roots[3]);
                    const tcl_u8 *val_str = TO_PTR(context, var_val_offset);
                    tcl_u32 val_copy_len = t_slen(val_str);
                    if (write_pos + val_copy_len >= out_buf_size - 1) {
                        val_copy_len = out_buf_size - 1 - write_pos;
                    }
                    t_mcpy(out_buf + write_pos, val_str, val_copy_len);
                    write_pos += val_copy_len;
                }
            } else {
                out_buf = TO_PTR(context, context->tmp_roots[3]);
                if (write_pos < out_buf_size - 1) {
                    out_buf[write_pos++] = '$';
                }
            }
        } else {
            ...
            out_buf[write_pos++] = cur_char;
            read_pos++;
        }
    }
    out_buf = TO_PTR(context, context->tmp_roots[3]);
    out_buf[write_pos] = 0;
    tcl_u32 result_offset = context->tmp_roots[3];
    context->tmp_roots[3] = TCL_NULL;
...
static tcl_u32 tcl_clone_slice(TclCtx *context, tcl_u32 source_offset, tcl_u32 slice_start, tcl_u32 slice_len) { /* 函数入口：克隆字符串切片 */
    ...
    if (clone_offset == TCL_NULL) { /* 内存不足时返回失败 */
        context->tmp_roots[15] = TCL_NULL;
        return TCL_NULL; /* 向上层返回分配失败 */
    } /* 结束分配失败分支 */
    source_offset = context->tmp_roots[15];
    context->tmp_roots[15] = TCL_NULL;
    const tcl_u8 *source_ptr = TO_PTR(context, source_offset); /* 获取源字符串物理指针 */
...
static tcl_u32 tcl_concat_three(TclCtx *context, tcl_u32 prefix_offset, tcl_u32 middle_offset, tcl_u32 suffix_offset) { /* 函数入口：执行三段字符串拼接 */
    ...
    prefix_offset = context->tmp_roots[12];
    middle_offset = context->tmp_roots[13];
    suffix_offset = context->tmp_roots[14];
    context->tmp_roots[12] = context->tmp_roots[13] = context->tmp_roots[14] = TCL_NULL;
...
static tcl_i32 tcl_cmd_lindex(TclCtx *context, tcl_i32 argument_count, tcl_u32 *argument_values) {
    ...
    while (*list_scan_ptr) { /* 线性扫描整个列表字符串 */
        ...
        if (current_element_idx == target_index) { /* 检查当前扫描到的元素是否命中目标索引 */
            tcl_u32 element_start_idx = element_start_pos - (const tcl_u8 *)TO_PTR(context, argument_values[1]);
