...
static tcl_u32 tcl_str_interp(TclCtx *context, const tcl_u8 *src_str, tcl_u32 src_len) {
    ...
    tcl_i32 bracket_depth = 0; /* 方括号深度计数器：用于保护 [] 内部的 $ 延迟到子命令阶段再展开 */
    /* 第二步：逐字符扫描，执行替换 */
    while (read_pos < src_len && write_pos < out_buf_size - 1) { /* 主扫描循环 */
        ...
        if (cur_char == '\\' && read_pos + 1 < src_len) { /* 转义序列检测 */
            ...
            if (bracket_depth > 0) { /* [] 内部变量禁止提前展开，保持延迟求值语义 */
                out_buf[write_pos++] = '$'; /* 原样写入字面量 $ */
                read_pos++; /* 仅推进一个字符，交由子命令阶段处理变量 */
                continue; /* 继续处理下一字符 */
            } /* 结束 [] 内 $ 保护 */
            read_pos++; /* 跳过 $ 符号 */
            ...
            if (cur_char == '[') { /* 识别左方括号并进入子命令上下文 */
                bracket_depth++; /* 深度自增 */
            } else if (cur_char == ']' && bracket_depth > 0) { /* 识别右方括号并退出一层上下文 */
                bracket_depth--; /* 深度自减 */
            } /* 结束方括号深度维护 */
