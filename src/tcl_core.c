...
static tcl_i32 tcl_cmd_if(TclCtx *context, tcl_i32 argument_count, tcl_u32 *argument_values) {
    ...
    active_frame_ptr->cond = argument_values[1]; /* 将待评估的条件表达式脚本偏移量存入栈帧 */
...
static tcl_i32 tcl_cmd_while(TclCtx *context, tcl_i32 argument_count, tcl_u32 *argument_values) {
    ...
    active_frame_ptr->cond = argument_values[1]; /* 设置循环判定条件脚本的物理偏移 */
...
static tcl_i32 tcl_cmd_expr(TclCtx *context, tcl_i32 argument_count, tcl_u32 *argument_values) {
    ...
    if (expression_script_ptr[0] == '{') { /* 如果脚本以花括号开始，视为需要求值的整体 */
        ...
        sub_frame_ptr->flags = FRAME_SHARE_SCOPE; /* 关键：表达式必须在父级作用域内执行以便访问变量 */
        ...
        switch (frame->state) { /* 基于当前帧内部状态寄存器的值，进行逻辑分支跳转 */
            case ST_TOKENIZE: { /* 分词状态：该阶段负责从当前的 pc 位置开始，识别出命令名称、参数及注释，处理物理分隔符 */
                ...
                while (script_ptr[frame->pc] && frame->argc < MAX_ARGS) { /* 设定 MAX_ARGS 阈值以防止 argv 数组越界 */
                    ...
                    /* 空行：逻辑分段 - Token 类型分发 */
                    if (script_ptr[frame->pc] == '{') { /* 特殊语法：处理由花括号包裹的“字面量” Token（不执行内部替换） */
                        tcl_i32 depth = 1; /* 初始化嵌套计数器：当前探测到一个左花括号，深度为 1 */
                        frame->pc++; /* 跨过左花括号引导符，进入实际的内容区域 */
                        start_pos = frame->pc; /* 重新修正 Token 内容的实际起始位置，剥离外层花括号 */
                        while (script_ptr[frame->pc] && depth > 0) { /* 迭代搜索匹配的右花括号，支持多层嵌套逻辑 */
                            if (script_ptr[frame->pc] == '{') { /* 探测到内部嵌套的起始符 */
                                depth++; /* 逻辑深度递增，确保后续能正确匹配到最外层的结束符 */
                            } else if (script_ptr[frame->pc] == '}') { /* 探测到可能的闭合符 */
                                depth--; /* 逻辑深度递减 */
                            } /* 结束嵌套深度判定 */
                            if (depth > 0) { /* 若尚未回归至零深度（即未触及最外层配对符号） */
                                frame->pc++; /* 持续推进游标，收集花括号内部的所有原始文本字符 */
                            } /* 结束字符收集判定 */
                        } /* 结束配对花括号搜索循环 */
                        tcl_u32 len = frame->pc - start_pos; /* 计算被剥离花括号后的核心内容长度 */
                        tcl_u32 allocated = tcl_alc_p(context, len + 1); /* 在 Arena 持久化区分配内存，用于存放该 Token 的副本 */
                        if (allocated != TCL_NULL) { /* 内存分配成功安全性校验 */
                            frame = TO_PTR(context, context->curr_f); /* 核心机制：分配后必须重新映射物理指针，应对潜在的 GC 内存压缩 */
                            script_ptr = TO_PTR(context, frame->script); /* 同步刷新脚本对象的物理基址 */
                            t_mcpy(TO_PTR(context, allocated), script_ptr + start_pos, len); /* 执行物理搬运：将花括号内容拷贝至新空间 */
                            ((tcl_u8*)TO_PTR(context, allocated))[len] = 0; /* 在 Token 副本末尾强制写入 null 结束符 */
                            frame->argv[frame->argc++] = allocated; /* 将新生成的 Token 对象逻辑偏移量存入参数数组，并增加计数 */
                        } /* 结束 Token 持久化处理 */
                        if (script_ptr[frame->pc] == '}') { /* 游标后处理：若 pc 停留在结束花括号上 */
                            frame->pc++; /* 跨过结束符，确保下一次分词从其后的字符开始 */
                        } /* 结束花括号闭合符处理 */
            ...
            case ST_EXPAND: { /* 指令展开阶段：负责处理参数数组中的动态内容，如 $变量 和 [子命令] */
                ...
                frame->state = ST_EXECUTE; /* 所有参数均已完成动态替换，迁移至指令执行阶段 */
            ...
            case ST_IF_COND: { /* 分支指令：if 条件求值准备阶段 */
                ...
                cond_frame->flags = FRAME_SHARE_SCOPE; /* 标记为共享作用域帧 */
            ...
            case ST_COND: { /* 循环指令：while 迭代的条件探测起点 */
                ...
                cond_frame->state = ST_TOKENIZE; /* 子帧进入初始化分词态 */
                cond_frame->flags = FRAME_SHARE_SCOPE;  /* 共享标志位 */
