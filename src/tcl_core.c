...
static tcl_i32 tcl_cmd_expr(TclCtx *context, tcl_i32 argument_count, tcl_u32 *argument_values) {
    ...
    if (expression_script_ptr[0] == '{') { /* 如果脚本以花括号开始，视为需要求值的整体 */
        ...
        sub_frame_ptr->parent = context->curr_f; /* 物理返回路径：计算完后回到父帧进行结果归约 */
        sub_frame_ptr->scope = context->curr_f; /* 逻辑作用域：表达式可以访问定义处的局部变量 */
...
void tcl_init(void *arena_ptr, tcl_i32 total_size) {  /* 函数入口：接收由宿主环境预分配的连续物理内存块首地址 */
    ...
    tcl_register_c_cmd((tcl_u8*)"global", tcl_cmd_global);  /* 核心指令注册：全局变量声明指令 global，使 proc 内部可读写全局作用域变量 */
