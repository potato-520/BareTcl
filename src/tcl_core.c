...
typedef struct {                    /* 定义 TclFrame 执行栈帧结构体 */
    ...
    tcl_u32 parent;                 /* 父执行栈帧的逻辑偏移量，用于退出作用域时恢复 */
...
static tcl_u32 tcl_find_var_node(TclCtx *context, tcl_u32 search_frame_offset, const tcl_u8 *target_name) { /* 函数入口：检索变量存储节点 */
    while (search_frame_offset != TCL_NULL) { /* 沿着执行栈帧链从当前活跃点向上层作用域回溯 */
        ...
        if (frame_ptr->flags & FRAME_SHARE_SCOPE) { /* 判断当前帧是否处于共享作用域模式（如 if/while/eval 块内部） */
            search_frame_offset = frame_ptr->parent; /* 允许跨越边界，继续在其逻辑父级栈帧中检索 */
        } else {                    /* 触及具有独立命名空间的过程/函数作用域边界 */
            break;                  /* 禁止向上穿透查找局部变量，以符合 Tcl 作用域隔离规范 */
        }                           /* 穿透查找判定结束 */
    }                               /* 动态调用链回溯结束 */
...
static tcl_i32 tcl_set_var(TclCtx *context, tcl_u32 frame_offset, tcl_u32 name_offset, tcl_u32 value_offset) {
    ...
    while (target_frame_offset != TCL_NULL) { /* 循环向上寻找作用域边界 */
        TclFrame *parent_frame = TO_PTR(context, target_frame_offset); /* 获取栈帧指针 */
        if (!(parent_frame->flags & FRAME_SHARE_SCOPE)) { /* 找到真正的非共享局部作用域边界 */
            break; /* 到达边界，跳出循环 */
        } /* 结束作用域属性检查 */
        target_frame_offset = parent_frame->parent; /* 向上层栈帧递归回溯 */
    } /* 结束作用域穿透循环 */
...
static tcl_i32 tcl_cmd_if(TclCtx *context, tcl_i32 argument_count, tcl_u32 *argument_values) {
    ...
    /* 处理 else 分支 */
    if (argument_count >= 5) {
        const tcl_u8 *else_kw = TO_PTR(context, argument_values[3]);
        if (t_scmp(else_kw, (tcl_u8*)"else") == 0) {
            /* 使用临时根保护 else_body，稍后在 ST_IF_BODY 会用到 */
            context->tmp_roots[11] = argument_values[4];
        }
    } else {
        context->tmp_roots[11] = TCL_NULL;
    }
...
static tcl_i32 tcl_cmd_expr(TclCtx *context, tcl_i32 argument_count, tcl_u32 *argument_values) {
    ...
    if (expression_script_ptr[0] == '{') { /* 如果脚本以花括号开始，视为需要求值的整体 */
        ...
        /* 在对象池中分配临时物理空间，用于存储剥离外层花括号后的纯净表达式脚本 */
        context->tmp_roots[0] = tcl_alc_p(context, script_length); /* 执行分配 */
        if (context->tmp_roots[0] == TCL_NULL) { /* 检查分配是否成功 */
            return TCL_ERROR; /* 分配失败报错 */
        } /* 结束分配检查 */
        tcl_u8 *stripped_script_ptr = TO_PTR(context, context->tmp_roots[0]); /* 定位剥离后的缓冲区指针 */
        t_mcpy(stripped_script_ptr, expression_script_ptr + 1, script_length - 2); /* 物理拷贝内部内容，移除 {} */
        stripped_script_ptr[script_length - 2] = 0; /* 在字符串末尾写入物理结束符 */
        /* 为剥离后的表达式分配并初始化一个新的执行栈帧，用于递归求值流程 */
        tcl_u32 sub_expression_frame_offset = tcl_alc_t(context, sizeof(TclFrame)); /* 分配栈帧空间 */
        if (sub_expression_frame_offset == TCL_NULL) { /* 检查栈帧分配结果 */
            return TCL_ERROR; /* 返回分配失败错误 */
        } /* 结束栈帧分配检查 */
        TclFrame *sub_frame_ptr = TO_PTR(context, sub_expression_frame_offset); /* 获取子结界物理指针 */
        sub_frame_ptr->script = context->tmp_roots[0]; /* 将剥离后的脚本绑定至新栈帧 */
        sub_frame_ptr->pc = 0; /* 重置子结界的程序计数器到起始点 */
        sub_frame_ptr->vars = TCL_NULL; /* expr 不应创建独立的局部变量表 */
        sub_frame_ptr->parent = context->curr_f; /* 建立父子栈帧调用链路 */
        sub_frame_ptr->state = ST_TOKENIZE; /* 设置子结界的初始状态为分词阶段 */
        sub_frame_ptr->flags = FRAME_SHARE_SCOPE | FRAME_IS_EXPR; /* 关键：表达式共享父级作用域并开启归约模式 */
        sub_frame_ptr->cond = TCL_NULL; /* 初始化条件字段为空 */
        sub_frame_ptr->body = TCL_NULL; /* 初始化主体字段为空 */
        sub_frame_ptr->result = TCL_NULL; /* 初始化执行结果为空 */
        sub_frame_ptr->argc = 0; /* 重置参数计数器 */
        sub_frame_ptr->exp_idx = 0; /* 重置展开索引 */
        context->tmp_roots[0] = TCL_NULL; /* 任务完成，释放对剥离脚本的临时根保护 */
        /* 挂起当前栈帧并设置状态为恢复态，以便接收来自子结界的计算返回值 */
        ((TclFrame*)TO_PTR(context, context->curr_f))->state = ST_RESUME; /* 更新父帧状态 */
        context->curr_f = sub_expression_frame_offset; /* 切换当前的执行上下文至表达式子结界 */
        return TCL_YIELD;       /* 抛出让步信号，驱动 FSM 进入新结界执行循环 */
    } /* 结束延迟求值逻辑分支 */
    /* 基础三元运算格式实现：处理 expr <v1> <op> <v2> 模式 */
    if (argument_count == 4) { /* 场景：标准操作数、操作符三元组 */
        tcl_i32 operand_left = t_atoi(TO_PTR(context, argument_values[1])); /* 将左侧操作数转换为整型 */
        tcl_i32 operand_right = t_atoi(TO_PTR(context, argument_values[3])); /* 将右侧操作数转换为整型 */
        const tcl_u8 *operator_str = TO_PTR(context, argument_values[2]); /* 获取运算符字符串指针 */
        tcl_i32 calculation_result = 0; /* 初始化本地计算结果变量 */
        /* 模式匹配并执行具体的算术或逻辑比较指令 */
        if (t_scmp(operator_str, (tcl_u8*)"==") == 0) calculation_result = (operand_left == operand_right); /* 相等判定 */
        else if (t_scmp(operator_str, (tcl_u8*)"!=") == 0) calculation_result = (operand_left != operand_right); /* 不等判定 */
        else if (t_scmp(operator_str, (tcl_u8*)">") == 0) calculation_result = (operand_left > operand_right); /* 大于判定 */
        else if (t_scmp(operator_str, (tcl_u8*)">=") == 0) calculation_result = (operand_left >= operand_right); /* 大于等于 */
        else if (t_scmp(operator_str, (tcl_u8*)"<") == 0) calculation_result = (operand_left < operand_right); /* 小于判定 */
        else if (t_scmp(operator_str, (tcl_u8*)"<=") == 0) calculation_result = (operand_left <= operand_right); /* 小于等于 */
        else if (t_scmp(operator_str, (tcl_u8*)"+") == 0) calculation_result = (operand_left + operand_right); /* 加法运算 */
        else if (t_scmp(operator_str, (tcl_u8*)"-") == 0) calculation_result = (operand_left - operand_right); /* 减法运算 */
        else if (t_scmp(operator_str, (tcl_u8*)"*") == 0) calculation_result = (operand_left * operand_right); /* 乘法运算 */
        else if (t_scmp(operator_str, (tcl_u8*)"/") == 0) calculation_result = (operand_right ? operand_left / operand_right : 0); /* 除法，带除零保护 */
        else if (t_scmp(operator_str, (tcl_u8*)"%") == 0) calculation_result = (operand_right ? operand_left % operand_right : 0); /* 取模，带除零保护 */
        /* 在堆池中分配 12 字节，用于存放最终计算结果的整数字符串表示 */
        tcl_u32 result_string_offset = tcl_alc_p(context, 12); /* 分配缓冲区 */
        if (result_string_offset != TCL_NULL) { /* 检查分配是否成功 */
            t_itoa(calculation_result, TO_PTR(context, result_string_offset)); /* 将整型结果格式化为字符串 */
            context->result = result_string_offset; /* 将生成的字符串偏移存入解释器结果区 */
        } /* 结束结果分配检查 */
    } else {                    /* 降级场景：若仅传一个参数，直接透传作为结果（多见于单值求值） */
...
static tcl_i32 tcl_cmd_eval(TclCtx *context, tcl_i32 argument_count, tcl_u32 *argument_values) {
    ...
    eval_frame_ptr->parent = context->curr_f; /* 绑定父子帧关系以维持调用栈 */
...
static tcl_i32 tcl_cmd_catch(TclCtx *context, tcl_i32 argument_count, tcl_u32 *argument_values) {
    ...
    sub_frame_ptr->parent = context->curr_f; /* 绑定父子关系链 */
...
static tcl_i32 tcl_cmd_upvar(TclCtx *context, tcl_i32 argument_count, tcl_u32 *argument_values) {
    ...
    /* 识别可选的 level 参数，如 upvar 2 ... */
    const tcl_u8 *first_param_ptr = TO_PTR(context, argument_values[1]); /* 获取首个参数内容 */
    if (argument_count > 3 && first_param_ptr[0] != '#' && !(first_param_ptr[0] >= '0' && first_param_ptr[0] <= '9')) { /* 判断层级格式 */
        /* 第一个参数既不是绝对层级也不是数字，推定为省略 level 默认向上追溯 1 层 */
    } else if (argument_count > 3) { /* 场景：用户显式提供了回溯层级深度 */
        back_level = t_atoi(first_param_ptr); /* 将层级字符串转换为整型偏移 */
        argument_start_index = 2; /* 参数偏移量递增，跳过已处理的 level 字段 */
    } /* 结束层级参数判定 */
    /* 执行物理栈帧的回溯查找 */
    tcl_u32 target_frame_offset = context->curr_f; /* 从当前执行帧启动回溯流程 */
    /* 通过遍历 TclFrame 结构中的 parent 指针，实现沿调用链向上回溯。
       每一步回溯都将当前帧偏移量替换为其父帧的偏移量，直到达到指定的层级深度或触及根作用域（NULL）。
       这是 Tcl 实现跨作用域变量访问的核心机制。 */
    for (tcl_i32 step = 0; step < back_level && target_frame_offset != TCL_NULL; step++) { /* 按深度向上层爬升 */
        target_frame_offset = ((TclFrame*)TO_PTR(context, target_frame_offset))->parent; /* 定位至父级栈帧偏移量 */
    } /* 结束栈帧回溯循环 */
    context->tmp_roots[12] = argument_values[argument_start_index];     /* 在分配操作前保护目标变量名 */
    context->tmp_roots[13] = argument_values[argument_start_index + 1]; /* 保护本地即将创建的链接变量名 */
    /* 在确定的目标物理栈帧中检索指定的变量节点 */
    tcl_u32 found_target_node = tcl_find_var_node(context, target_frame_offset, TO_PTR(context, context->tmp_roots[12])); /* 执行检索 */
...
static tcl_i32 tcl_cmd_uplevel(TclCtx *context, tcl_i32 argument_count, tcl_u32 *argument_values) {
    ...
    tcl_u32 target_parent_offset = context->curr_f; /* 初始指向当前帧 */
    tcl_i32 back_steps = 1; /* 默认追溯步数 */
    tcl_i32 script_arg_index = 1; /* 默认脚本参数索引 */
    /* 识别可选的层级参数 */
    const tcl_u8 *first_argument_ptr = TO_PTR(context, argument_values[1]); /* 获取首个参数指针 */
    if (argument_count > 2 && first_argument_ptr[0] != '{' && first_argument_ptr[0] != '[') { /* 判断是否提供 level */
        back_steps = t_atoi(first_argument_ptr); /* 解析用户指定的步数 */
        script_arg_index = 2; /* 脚本内容参数后移 */
    } /* 结束层级参数判定 */
    /* 向上回溯指定层级的物理调用链 */
    /* 此循环通过连续访问当前栈帧的 parent 字段，实现调用栈的“向后跳转”。
       通过修改 target_parent_offset，我们将脚本的执行环境定位到上层的物理栈帧。
       这是实现 Tcl 动态作用域劫持的关键技术手段。 */
    for (tcl_i32 step = 0; step < back_steps && target_parent_offset != TCL_NULL; step++) { /* 按步数向上回溯 */
        target_parent_offset = ((TclFrame*)TO_PTR(context, target_parent_offset))->parent; /* 更新目标偏移为父级偏移 */
    } /* 结束栈帧回溯循环 */
    /* 分配并初始化新栈帧用于执行脚本 */
    ...
    new_frame_ptr->parent = target_parent_offset; /* 核心：重定向脚本的执行父节点，实现跨作用域执行 */
...
tcl_i32 tcl_eval(TclCtx *context, const tcl_u8 *script) { /* 核心解释器引擎入口：接收上下文与脚本字符串，采用状态机驱动实现无递归求值 */
    ...
    frame->parent = context->curr_f; /* 建立调用链回溯：记录父执行帧位置，虽然此处为顶层，但逻辑需保持一致 */
        ...
        switch (frame->state) { /* 基于当前帧内部状态寄存器的值，进行逻辑分支跳转 */
            ...
            case ST_EXPAND: { /* 指令展开阶段：负责处理参数数组中的动态内容，如 $变量 和 [子命令] */
                while (frame->exp_idx < frame->argc) { /* 线性迭代处理 argv 中的每一个 Token */
                    ...
                    } else if (arg_ptr[0] == '{' || arg_ptr[0] == '"') { /* 处理字面量：统一剥离定界符 */
                        tcl_u32 len = t_slen(arg_ptr);
                        if (len >= 2) {
                            tcl_u32 stripped = tcl_alc_p(context, len);
                            if (stripped != TCL_NULL) {
                                frame = TO_PTR(context, context->curr_f);
                                arg_ptr = TO_PTR(context, frame->argv[frame->exp_idx]);
                                t_mcpy(TO_PTR(context, stripped), arg_ptr + 1, len - 2);
                                ((tcl_u8*)TO_PTR(context, stripped))[len - 2] = 0;
                                frame->argv[frame->exp_idx] = stripped;
                            }
                        }
                    } /* 结束命令替换判定 */
                ...
                if (!found) { /* 若在 C 内建指令集中未命中，则尝试在全局作用域寻找脚本定义的 Proc */
                    ...
                    if (body_offset != TCL_NULL) { /* 若已识别到有效的脚本过程定义 */
                        ...
                        if (new_frame_offset == TCL_NULL) { /* 栈内存安全性校验 */
                            ...
                            new_frame->parent = context->curr_f; /* 建立调用链回溯，指向当前的调用者帧 */
                            ...
                            while (arg_idx < frame->argc) { /* 循环处理调用方传入的每一个有效实参 */
                                ...
                                /* 绑定操作：在新建的 Proc 子帧作用域内，将形参名关联到调用方传入的实参对象 */
            ...
            case ST_EXPR_REDUCE: { /* 表达式归约阶段：负责对已展开的 Token 进行逻辑或算术缩减，而非作为指令执行 */
                ...
                if (frame->argc == 0) { /* 边界情况：无 Token */
                    ...
                    context->result = frame->argv[0]; 
                } else if (frame->argc == 3) { /* 三元模式：处理 <v1> <op> <v2>，如 1 == 1 或 [info commands] == "" */
                    tcl_i32 op_left_int = t_atoi(TO_PTR(context, frame->argv[0])); /* 将左操作数文本转换为整型数值 */
                    tcl_i32 op_right_int = t_atoi(TO_PTR(context, frame->argv[2])); /* 将右操作数文本转换为整型数值 */
                    const tcl_u8 *op_str = TO_PTR(context, frame->argv[1]); /* 获取运算符物理指针 */
                    tcl_i32 res = 0; /* 本地暂存计算得出的布尔或算术结果 */
                    /* 分支分发：执行具体的比较或数学逻辑指令 */
                    if (t_scmp(op_str, (tcl_u8*)"==") == 0) { 
                        /* 增强判定：同时支持数值相等与字符串层面的字面量精确匹配判定 */
                        res = (t_scmp(TO_PTR(context, frame->argv[0]), TO_PTR(context, frame->argv[2])) == 0); 
                    } else if (t_scmp(op_str, (tcl_u8*)"!=") == 0) {
                        res = (t_scmp(TO_PTR(context, frame->argv[0]), TO_PTR(context, frame->argv[2])) != 0);
                    } else if (t_scmp(op_str, (tcl_u8*)">") == 0) res = (op_left_int > op_right_int);
                    else if (t_scmp(op_str, (tcl_u8*)">=") == 0) res = (op_left_int >= op_right_int);
                    else if (t_scmp(op_str, (tcl_u8*)"<") == 0) res = (op_left_int < op_right_int);
                    else if (t_scmp(op_str, (tcl_u8*)"<=") == 0) res = (op_left_int <= op_right_int);
                    else if (t_scmp(op_str, (tcl_u8*)"+") == 0) res = (op_left_int + op_right_int);
                    ...
                    else if (t_scmp(op_str, (tcl_u8*)"/") == 0) res = (op_right_int ? op_left_int / op_right_int : 0);
                    /* 将计算结果重新转化为字符串对象并存入结果区，以供父帧通过 tcl_get_result 读取 */
                    tcl_u32 res_str_off = tcl_alc_p(context, 12); 
                    if (res_str_off != TCL_NULL) {
                        t_itoa(res, TO_PTR(context, res_str_off)); 
                        context->result = res_str_off;
                    } /* 结束结果字符串化 */
                } else { /* 兜底逻辑：对于目前尚不支持的复杂多参数表达式，透传最后一个 Token 偏移量 */
                    context->result = frame->argv[frame->argc - 1]; 
                }
            ...
            case ST_IF_COND: { /* 分支指令：if 条件求值准备阶段 */
                ...
                cond_frame->parent = context->curr_f; /* 建立返回链，求值后回到当前帧 */
            ...
            case ST_IF_BODY: { /* 分支指令：判定条件布尔结果并决策是否执行主体 (body) */
                ...
                if (result_string[0] && result_string[0] != '0') { /* 符合 Tcl 标准布尔哲学：非空且首字节非 '0' 为逻辑真 */
                    ...
                    body_frame->parent = context->curr_f; /* 执行完后返回本 if 帧进行收尾 */
                    ...
                    frame->state = ST_TOKENIZE; /* 直接切换回解析态，跳过当前 if 的 body 脚本 */
                } /* 结束逻辑分支分发 */
            ...
            case ST_COND: { /* 循环指令：while 迭代的条件探测起点 */
                ...
                cond_frame->parent = context->curr_f; /* 建立返回链接 */
                cond_frame->state = ST_TOKENIZE; /* 子帧进入初始化分词态 */
            ...
            case ST_LOOP: { /* 循环执行阶段：根据条件探测结果，决策是否进入或继续执行 while 循环体 */
                ...
                if (result_string[0] && result_string[0] != '0') { /* 遵循 Tcl 哲学：判定结果非空且首字符非 '0' 即为真 */
                    ...
                    body_frame->parent = context->curr_f; /* 循环体完后应返回当前 while 控制帧 */
                    body_frame->state = ST_TOKENIZE; /* 子帧初始进入分词态 */
...
void tcl_init(void *arena_ptr, tcl_i32 total_size) {  /* 函数入口：接收由宿主环境预分配的连续物理内存块首地址 */
    ...
    tcl_register_c_cmd((tcl_u8*)"lrange", tcl_cmd_lrange);  /* 列表指令注册：截取子列表指令 lrange */
    tcl_register_c_cmd((tcl_u8*)"unset", tcl_cmd_unset);  /* 变量指令注册：销毁指定变量指令 unset */
    tcl_register_c_cmd((tcl_u8*)"info", tcl_cmd_info);  /* 系统指令注册：运行时状态自省指令 info */
