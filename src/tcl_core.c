        ...
        switch (frame->state) { /* 基于当前帧内部状态寄存器的值，进行逻辑分支跳转 */
            ...
            case ST_IF_BODY: { /* 分支指令：判定条件布尔结果并决策是否执行主体 (body) */
                ...
                if (result_string[0] && result_string[0] != '0') { /* 符合 Tcl 标准布尔哲学：非空且首字节非 '0' 为逻辑真 */
                    ...
                    if (context->tmp_roots[11] != TCL_NULL) { /* 存在 else body，需要执行 */
                        tcl_u32 else_body_script = context->tmp_roots[11]; /* 获取 else 脚本偏移量 */
                        context->tmp_roots[11] = TCL_NULL; /* 立即解除 GC 保护根，防止悬挂 */
                        ...
                        context->tmp_roots[11] = TCL_NULL; /* 确保保护根清空 */
