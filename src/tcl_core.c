...
static tcl_u32 tcl_concat_three(TclCtx *context, tcl_u32 prefix_offset, tcl_u32 middle_offset, tcl_u32 suffix_offset) { /* 函数入口：执行三段字符串拼接 */
    const tcl_u8 *prefix_ptr = TO_PTR(context, prefix_offset); /* 获取前缀字符串物理指针 */
    const tcl_u8 *middle_ptr = TO_PTR(context, middle_offset); /* 获取中段字符串物理指针 */
    const tcl_u8 *suffix_ptr = TO_PTR(context, suffix_offset); /* 获取后缀字符串物理指针 */
    tcl_u32 prefix_len = t_slen(prefix_ptr); /* 计算前缀长度 */
    tcl_u32 middle_len = t_slen(middle_ptr); /* 计算中段长度 */
    tcl_u32 suffix_len = t_slen(suffix_ptr); /* 计算后缀长度 */
    ...
    t_mcpy(merged_ptr, prefix_ptr, prefix_len); /* 写入前缀部分 */
    t_mcpy(merged_ptr + prefix_len, middle_ptr, middle_len); /* 写入中段部分 */
    t_mcpy(merged_ptr + prefix_len + middle_len, suffix_ptr, suffix_len); /* 写入后缀部分 */
...
static tcl_i32 tcl_cmd_expr(TclCtx *context, tcl_i32 argument_count, tcl_u32 *argument_values) {
    ...
    if (argument_count == 2) { /* 单参数模式：剥括号后需要展开并在新帧求值 */
        ...
        tcl_u32 interpolated_offset = tcl_str_interp(context, inner_expr_ptr, inner_expr_len); /* 先做变量插值并保留方括号延迟语义 */
