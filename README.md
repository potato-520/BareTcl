...
当前 BareTcl 在 ESP32 移植版本中已完美支持**物理串口控制台 (UART/USB-JTAG)**与**浏览器 Web 终端 (WebSocket & xterm.js)**的双通道实时同步运行！您既可以通过传统的物理串口进行指令调试，也可以直接打开网页在极具科技感的高级暗黑磨砂面板中以图形化按键开关板载继电器、实时查看命令行和网络 Ping 状态。

![BareTcl Web Console](./docs/websocket_BareTcl.png)
