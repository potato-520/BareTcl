...
- **ESP32 芯片级原生移植**：在 ESP32-C3 (ESP-IDF 架构) 上实现原生 C 全功能移植（参见 [ESP32_ports](file:///home/chenming/BareTcl/ESP32_ports)）。包含非阻塞串口 REPL、VFS 无缓冲实时输入、软硬件看门狗安全让出（`TCL_YIELD_HOOK`）以及物理 GPIO (继电器/按键) 的 C 扩展绑定。
